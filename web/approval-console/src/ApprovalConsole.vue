...
<template>
  <section class="approval-case" :aria-labelledby="`approval-${approvalCase.case_id}`">
    ...
    <div class="button-row">
      <button type="button" :disabled="!pending || !reason.trim() || busy" @click="submit('APPROVE')">{{ labels.approve }}</button>
      <button type="button" class="danger" :disabled="!pending || !reason.trim() || busy" @click="submit('REJECT')">{{ labels.reject }}</button>
    </div>
