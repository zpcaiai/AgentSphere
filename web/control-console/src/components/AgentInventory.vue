  ...
  <section aria-labelledby="agent-inventory-title">
    ...
    <div v-if="page" class="table-scroll" tabindex="0">
      <table>
        ...
        <thead><tr><th scope="col">Agent ID</th><th scope="col">Version</th><th scope="col">Posture</th><th scope="col">Safe summary</th></tr></thead>
        <tbody>
          <tr v-for="agent in page.items" :key="agent.agent_id">
            <td>{{ agent.agent_id }}</td><td>{{ agent.version ?? '—' }}</td><td>{{ agent.posture ?? '—' }}</td><td>{{ agent.safe_summary ?? '—' }}</td>
          </tr>
          <tr v-if="page.items.length === 0"><td colspan="4">{{ locale === 'en-US' ? 'No agents' : '暂无 Agent' }}</td></tr>
        </tbody>
      </table>
