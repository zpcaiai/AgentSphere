...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let activation_guardian = Arc::new(ActivationGuardian::new(ActivationGuardianConfig {
        release_id: required_env("AGENT_TRUST_EXECUTION_ACTIVATION_RELEASE_ID")?,
        receipt_path: PathBuf::from(required_env(
            "AGENT_TRUST_EXECUTION_ACTIVATION_RECEIPT_FILE",
        )?),
        max_staleness_seconds: required_env(
