//! Verification of request-bound human assertions shared with the Java BFF and PEP.
...
pub enum PrincipalAssertionError {
    #[error("ENTERPRISE_PRINCIPAL_CONFIGURATION_INVALID")]
    ConfigurationInvalid,
    #[error("ENTERPRISE_PRINCIPAL_ASSERTION_INVALID")]
    AssertionInvalid,
}
...
impl HumanPrincipalKeyring {
    pub fn from_file(
        ...
        if !identifier(expected_audience, 256) {
...
impl HumanPrincipalVerificationKeyDocument {
    fn tenants_invalid(&self) -> bool {
        self.tenant_ids.is_empty()
            || self.tenant_ids.len() > 1_024
            || self.tenant_ids.iter().any(|value| {
