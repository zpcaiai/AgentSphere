...
use agent_trust_contracts::{
    APPROVAL_REVIEW_EVIDENCE_SCHEMA_VERSION, AUTHORITY_EVIDENCE_EVENT_REQUEST_SCHEMA_VERSION,
    ActionHash, ApprovalReviewEvidence, ApprovalReviewEvidenceIssueRequest,
...
use agent_trust_pack_supply_chain::server::{
    EvidenceEventKeyring, ExactPeerIdentity, ExactPeerIdentityAcceptor, SupplyDependency,
};
...
#[derive(Clone)]
struct ApiState {
    authority: DomainRuntimeAuthority,
    tokens: Arc<DomainTokenAuthorizer>,
    review_producer: Arc<HttpDomainRuntimePort>,
}
#[derive(Clone)]
struct ReadyState {
    authority: DomainRuntimeAuthority,
...
pub struct DomainTokenAuthorizer {
...
impl DomainTokenAuthorizer {
    pub fn from_file(
        ...
        let raw = read_private(path, 1, 1_048_576)?;
        let document: TokenDocument =
            serde_json::from_slice(&raw).map_err(|_| DomainAuthorityError::ConfigurationInvalid)?;
        if document.schema_version != "agenttrust.domain-runtime-token-bindings.v1"
        ...
        for binding in document.bindings {
            if !allowed.contains(&binding.client_identity)
                || !matches!(
                    ...
                    DOMAIN_EXECUTE_SCOPE
                        | DOMAIN_READ_SCOPE
                        | DOMAIN_RECOVER_SCOPE
                        | DOMAIN_APPROVAL_REVIEW_EVIDENCE_SCOPE
                )
                || !canonical_uuid(&binding.tenant_id)
                || !identifier(&binding.subject, 512)
                || !digest(&binding.token_sha256)
                || !physical.insert(binding.token_sha256.clone())
    ...
    fn authorize(
        ...
        scope: &str,
        headers: &HeaderMap,
    ) -> Result<String, DomainAuthorityError> {
        let token = single_header(headers, "authorization")
            ...
            .ok_or(DomainAuthorityError::PrincipalDenied)?;
        ...
        match subject {
            ...
            _ => Err(DomainAuthorityError::PrincipalDenied),
        }
    }
}
...
impl HttpDomainRuntimePort {
    pub fn new(
        ...
        if coordinator.name != "executor"
            || evidence.name != "evidence"
            || coordinator.endpoint == evidence.endpoint
            || !valid_https_root(&coordinator.endpoint)
            || !valid_https_root(&evidence.endpoint)
            || coordinator.token_file == evidence.token_file
    ...
    pub async fn issue_approval_review_evidence(
        ...
        let request = issue
            .to_authority_event(&self.evidence_client_identity, chrono::Utc::now())
        ...
        {
            return Err(DomainAuthorityError::DependencyUnavailable);
        }
...
impl DomainRuntimePort for HttpDomainRuntimePort {
    async fn execute(
        ...
        let response = self
            ...
            .post(
                self.coordinator
                    .endpoint
                    .join("v1/domain-runtime/effects")
                    .map_err(|_| DomainAuthorityError::ConfigurationInvalid)?,
            )
            .bearer_auth(read_token(&self.coordinator.token_file)?)
            .header(
                "X-AgentTrust-Tenant-Id",
                envelope.binding.tenant_id.to_string(),
            )
            .header("Idempotency-Key", &envelope.binding.idempotency_key)
            .header(
                "X-AgentTrust-Execution-Id",
                envelope.execution_id.to_string(),
            )
            .header("X-AgentTrust-Action-Hash", action_hash)
            .header(
                "X-AgentTrust-Authorization-Id",
                envelope.binding.authorization_id.to_string(),
            )
            .header(
                "X-AgentTrust-Authorization-Digest",
                &envelope.binding.authorization_digest,
            )
            .header(
                "X-AgentTrust-Policy-Decision-Digest",
                &envelope.binding.policy_decision_digest,
            )
            .header(
                "X-AgentTrust-Authorization-Evidence-Ref",
                &envelope.binding.authorization_evidence_ref,
            )
            .header(
                "X-AgentTrust-Authorization-Evidence-Digest",
                &envelope.binding.authorization_evidence_digest,
            )
            .header(
                "X-AgentTrust-Ledger-Execution-Id",
                envelope.binding.ledger_execution_id.to_string(),
            )
            .header(
                "X-AgentTrust-Ledger-Entry-Id",
                envelope.binding.ledger_event_id.to_string(),
            )
            .header(
                "X-AgentTrust-Ledger-Entry-Digest",
                &envelope.binding.ledger_event_digest,
            )
            .header("X-AgentTrust-Fence-Digest", &envelope.binding.fence_digest)
    ...
    async fn deliver_evidence(
        ...
        let task_id = Uuid::parse_str(payload_string(payload, "task_id", 36)?)
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
        let event_id = Uuid::parse_str(payload_string(payload, "event_id", 36)?)
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
        let ledger_execution_id =
            Uuid::parse_str(payload_string(payload, "ledger_execution_id", 36)?)
                .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
        let ledger_event_id = Uuid::parse_str(payload_string(payload, "ledger_event_id", 36)?)
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
        let action_hash = payload_string(payload, "action_hash", 64)?;
        let ledger_event_digest = payload_string(payload, "ledger_event_digest", 64)?;
        let fence_digest = payload_string(payload, "fence_digest", 64)?;
        let policy_decision_id = payload_string(payload, "policy_decision_id", 256)?;
        let policy_decision_digest = payload_string(payload, "policy_decision_digest", 64)?;
        let authorization_evidence_ref =
            payload_string(payload, "authorization_evidence_ref", 2048)?;
        let authorization_evidence_digest =
            payload_string(payload, "authorization_evidence_digest", 64)?;
        if [
            action_hash,
            ledger_event_digest,
            fence_digest,
        ...
        if occurred_at > requested_at + chrono::Duration::minutes(1)
            || requested_at > chrono::Utc::now() + chrono::Duration::minutes(1)
        {
            return Err(DomainAuthorityError::ReceiptInvalid);
        }
        let actor = payload_string(payload, "actor_subject", 512)?;
        let trace = payload_string(payload, "trace_id", 256)?;
        let span = payload_string(payload, "command_id", 256)?;
        let request = AuthorityEvidenceEventRequest {
            ...
            event: EvidenceEventDraft {
                ...
                safe_summary: "Domain Runtime authority outcome persisted".into(),
    ...
    async fn ready(&self) -> bool {
        dependency_ready(&self.client, &self.coordinator).await
            && dependency_ready(&self.client, &self.evidence).await
    }
}
...
async fn dependency_ready(client: &reqwest::Client, dependency: &SupplyDependency) -> bool {
    ...
    if !response.status().is_success()
        || response.content_length().is_some_and(|value| value > 4096)
    {
        return false;
    }
    ...
    !bytes.is_empty()
        && serde_json::from_slice::<DependencyReadiness>(&bytes)
            .is_ok_and(|value| value.ready && value.schema_version == dependency.readiness_schema)
}
...
pub fn router(
    ...
    Router::new()
        ...
        .route(
            ...
            post(issue_approval_review_evidence),
        )
        .route(
...
async fn data_ready(State(state): State<ApiState>) -> Result<Json<serde_json::Value>, ApiError> {
...
async fn execute(
    ...
    headers: HeaderMap,
    Json(body): Json<DomainExecutionEnvelope>,
) -> Result<Json<DomainExecutionResult>, ApiError> {
    exact_headers(&headers, &body)?;
    let subject = state.tokens.authorize(
        &peer,
        body.binding.tenant_id,
        DOMAIN_EXECUTE_SCOPE,
        &headers,
    )?;
    if subject != body.actor_subject {
        return Err(DomainAuthorityError::PrincipalDenied.into());
    }
    Ok(Json(state.authority.execute(body).await?))
}
async fn issue_approval_review_evidence(
    State(state): State<ApiState>,
    Extension(ExactPeerIdentity(peer)): Extension<ExactPeerIdentity>,
    headers: HeaderMap,
    Json(body): Json<ApprovalReviewEvidenceIssueRequest>,
) -> Result<Json<ApprovalReviewEvidence>, ApiError> {
    body.material
        .validate()
        .map_err(|_| DomainAuthorityError::RequestInvalid)?;
    let tenant = Uuid::parse_str(&body.material.tenant_id)
        .map_err(|_| DomainAuthorityError::RequestInvalid)?;
    ...
    if tenant.to_string() != body.material.tenant_id
        || single_header(&headers, "x-agenttrust-tenant-id")
            != Some(body.material.tenant_id.as_str())
    ...
    Ok(Json(
        state
            .review_producer
            .issue_approval_review_evidence(&body)
            .await?,
    ))
}
#[derive(Debug, Deserialize)]
...
) -> Result<Json<AuthoritativeDomainExecutionPage>, ApiError> {
    if required_header(&headers, "x-agenttrust-tenant-id")? != query.tenant_id.to_string() {
        return Err(DomainAuthorityError::PrincipalDenied.into());
    }
    state
        .tokens
        .authorize(&peer, query.tenant_id, DOMAIN_READ_SCOPE, &headers)?;
    Ok(Json(
        state
            .authority
            .authoritative_state(
...
async fn recover(
    ...
    if required_header(&headers, "x-agenttrust-tenant-id")? != tenant.to_string() {
        return Err(DomainAuthorityError::PrincipalDenied.into());
    }
    state
        .tokens
        .authorize(&peer, tenant, DOMAIN_RECOVER_SCOPE, &headers)?;
    let marked = state.authority.recover_expired(tenant, 100).await?;
    Ok(Json(
        serde_json::json!({"schema_version":"agenttrust.domain-runtime-recovery.v1","marked_unknown":marked}),
    ))
}
fn exact_headers(
    headers: &HeaderMap,
    body: &DomainExecutionEnvelope,
) -> Result<(), DomainAuthorityError> {
    let expected = [
        ("x-agenttrust-tenant-id", body.binding.tenant_id.to_string()),
        ("idempotency-key", body.binding.idempotency_key.clone()),
        ("x-agenttrust-execution-id", body.execution_id.to_string()),
        ("x-agenttrust-action-id", body.command_id.to_string()),
        (
            "x-agenttrust-authorization-id",
    ...
    if expected
        .into_iter()
        .any(|(name, value)| single_header(headers, name) != Some(value.as_str()))
    {
...
pub async fn serve(
    ...
    let acceptor = ExactPeerIdentityAcceptor::new(
        &config.tls_ca_file,
        &config.tls_certificate_file,
        &config.tls_private_key_file,
        config.allowed_client_identities,
    )
    .map_err(|_| DomainAuthorityError::ConfigurationInvalid)?;
...
async fn management_live() -> Json<serde_json::Value> {
    Json(serde_json::json!({"schema_version":DOMAIN_READINESS_SCHEMA,"live":true}))
}
...
async fn ready(authority: &DomainRuntimeAuthority) -> Result<Json<serde_json::Value>, ApiError> {
    if !authority.ready().await {
        return Err(DomainAuthorityError::DependencyUnavailable.into());
    }
    Ok(Json(
        serde_json::json!({"schema_version":DOMAIN_READINESS_SCHEMA,"ready":true,"database_ready":true,"executor_ready":true,"evidence_ready":true}),
    ))
}
struct ApiError(DomainAuthorityError);
impl From<DomainAuthorityError> for ApiError {
    fn from(value: DomainAuthorityError) -> Self {
        Self(value)
    }
}
impl IntoResponse for ApiError {
    fn into_response(self) -> Response {
        let status = match self.0 {
            DomainAuthorityError::PrincipalDenied => StatusCode::UNAUTHORIZED,
            DomainAuthorityError::RequestInvalid
            | DomainAuthorityError::Contract(_)
            | DomainAuthorityError::ReceiptInvalid => StatusCode::BAD_REQUEST,
            DomainAuthorityError::PackInactive
            | DomainAuthorityError::ApprovalInvalid
            | DomainAuthorityError::SupervisionInvalid
            | DomainAuthorityError::IdempotencyConflict
            | DomainAuthorityError::StateConflict => StatusCode::CONFLICT,
            DomainAuthorityError::OutcomeUnknown
            | DomainAuthorityError::DependencyUnavailable
            | DomainAuthorityError::ConfigurationInvalid => StatusCode::SERVICE_UNAVAILABLE,
        };
        (status,Json(serde_json::json!({"schema_version":"agenttrust.domain-runtime-error.v1","error":self.0.code()}))).into_response()
    }
}
fn required_header<'a>(
    headers: &'a HeaderMap,
    name: &'static str,
) -> Result<&'a str, DomainAuthorityError> {
    single_header(headers, name).ok_or(DomainAuthorityError::RequestInvalid)
}
...
fn read_private(path: &Path, minimum: u64, maximum: u64) -> Result<Vec<u8>, DomainAuthorityError> {
    let metadata =
        std::fs::symlink_metadata(path).map_err(|_| DomainAuthorityError::ConfigurationInvalid)?;
    if !path.is_absolute()
    ...
    std::fs::read(path).map_err(|_| DomainAuthorityError::ConfigurationInvalid)
}
fn constant_time_equal(first: &str, second: &str) -> bool {
    let key = hmac::Key::new(hmac::HMAC_SHA256, b"agenttrust-domain-token-compare-v1");
...
fn payload_string<'a>(
    ...
    payload
        ...
        .filter(|value| identifier(value, maximum))
        .ok_or(DomainAuthorityError::ReceiptInvalid)
}
fn parse_evidence_time(
    payload: &serde_json::Value,
    key: &str,
) -> Result<chrono::DateTime<chrono::Utc>, DomainAuthorityError> {
    let value = payload_string(payload, key, 64)?;
    chrono::DateTime::parse_from_rfc3339(value)
        .map(|parsed| parsed.with_timezone(&chrono::Utc))
        .map_err(|_| DomainAuthorityError::ReceiptInvalid)
}
fn read_token(path: &Path) -> Result<String, DomainAuthorityError> {
    let raw = read_private(path, 16, 8194)?;
    let value =
        std::str::from_utf8(&raw).map_err(|_| DomainAuthorityError::ConfigurationInvalid)?;
...
fn valid_https_root(value: &url::Url) -> bool {
    value.scheme() == "https"
