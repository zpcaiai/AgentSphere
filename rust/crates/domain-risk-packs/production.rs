...
impl DomainKind {
    pub fn pack_id(self) -> &'static str {
        match self {
            ...
            Self::Sensitive => "sensitive-interaction",
        }
    }
    fn expected_manifest(self) -> DomainPackManifest {
        match self {
            Self::Coding => coding::manifest(),
            Self::Industrial => industrial::manifest(),
            Self::Energy => energy::manifest(),
            Self::Medical => medical::manifest(),
            Self::Sensitive => sensitive::manifest(),
        }
    }
}
...
struct IndustrialCommand {
    ...
    interlock_digest: String,
    alarm_digest: String,
    approval_set_id: Option<Uuid>,
    supervision_id: Option<Uuid>,
}
...
struct IndustrialOperationCommand {
    ...
    asset_id: Uuid,
    stage: DeploymentStage,
    endpoint_manifest_digest: String,
    observed_resource_version: u64,
    state_digest: String,
    outcome_digest: String,
    maximum_age_ms: u64,
    scenario_digest: Option<String>,
    safe_stop_profile_digest: Option<String>,
...
struct EnergyCommand {
    ...
    asset_id: Uuid,
    algorithm: String,
    algorithm_manifest_digest: String,
    stage: DeploymentStage,
    setpoints_kw: Vec<f64>,
    confidence: f64,
    out_of_distribution: bool,
    requires_shadow_validation: bool,
    constraint_digest: String,
    forecast_digest: String,
    fallback_controller_digest: String,
...
struct EnergyObservationCommand {
    ...
    model_manifest_digest: Option<String>,
    forecast_valid_until: Option<DateTime<Utc>>,
}
...
struct MedicalReviewCommand {
    ...
    care_relationship_digest: String,
    recommendation_digest: String,
    clinical_evidence_digest: String,
    model_manifest_digest: String,
    risk_level: String,
    reviewer_subject: String,
    reviewer_role: String,
    reviewer_qualification_digest: String,
    principal_assertion_digest: String,
    decision: String,
    evidence_ref: String,
...
struct MedicalAssistCommand {
    ...
    data_policy_decision_digest: String,
    residency_decision_digest: String,
...
struct SensitiveHandoffCommand {
    ...
    dynamic_region_directory_digest: String,
    minimum_safe_context_digest: String,
    ordinary_agent_paused: bool,
    status: String,
}
...
struct SensitiveInteractionCommand {
    ...
    relationship_boundary_digest: String,
...
impl ProductionDomainPackContract {
    pub fn validate_manifest(
        ...
        verifier
            ...
            .map_err(|_| DomainProductionError::PackInvalid)?;
    ...
    pub fn authorize(
        ...
        let expected_current_state_version = envelope
            .binding
            .resource_version
            .saturating_sub(1)
            .to_string();
        ...
        let limited_physical_write =
            matches!(envelope.domain, DomainKind::Industrial | DomainKind::Energy)
                && envelope.deployment_stage == Some(DeploymentStage::LimitedWrite)
                && (envelope.operation.ends_with("commit")
                    || envelope.operation.ends_with("restore")
                    || envelope.operation.ends_with("activate"));
        if envelope.physical_supervision.is_some() != limited_physical_write
            || (!tool.approval_required
                && !limited_physical_write
                && !envelope.expert_approvals.is_empty())
        {
...
fn validate_typed_command(
    envelope: &DomainExecutionEnvelope,
    tool: &agent_trust_pack_supply_chain::PackToolDefinition,
) -> Result<(), DomainProductionError> {
    if envelope.safe_command.get("schema_version")
        != Some(&Value::String(DOMAIN_PACKS_SCHEMA_VERSION.into()))
    {
        return Err(DomainProductionError::CommandInvalid);
    }
    match envelope.domain {
        DomainKind::Coding => {
            let command: CodingCommand = serde_json::from_value(envelope.safe_command.clone())
                .map_err(|_| DomainProductionError::CommandInvalid)?;
            let mut paths = BTreeSet::new();
            let mut network = BTreeSet::new();
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || envelope.deployment_stage.is_some()
                || command.repository_ref != envelope.resource_key
                || !https_reference(&command.repository_ref)
                || !hex_length(&command.base_commit, &[40, 64])
                || !command.branch.starts_with("agent/")
                || command.branch.len() > 486
                || command.paths.is_empty()
                || command.paths.len() > 10_000
                || command
                    .paths
                    .iter()
                    .any(|path| !paths.insert(path.as_str()) || unsafe_repository_path(path))
                || command.command_template_id != tool.executor_template
                || command
                    .command_template_id
                    .to_ascii_lowercase()
                    .contains("shell")
                || command
                    .requested_network
                    .iter()
                    .any(|value| !network.insert(value.as_str()) || !identifier(value, 256))
                || !network.is_empty()
                || !digest(&command.dependency_lock_digest)
                || !reference(&command.rollback_ref)
                || command.effect_class != tool.effect_class
            {
                return Err(DomainProductionError::CommandInvalid);
            }
        }
        DomainKind::Industrial
            if matches!(
                envelope.operation.as_str(),
                "industrial.setpoint_prepare" | "industrial.setpoint_commit"
            ) =>
        {
            let command: IndustrialCommand = serde_json::from_value(envelope.safe_command.clone())
                .map_err(|_| DomainProductionError::CommandInvalid)?;
            let current = envelope
                ...
                .saturating_sub(1)
                .to_string();
            let limited_write = command.stage == DeploymentStage::LimitedWrite
                && envelope.operation == "industrial.setpoint_commit";
            let approval_matches = command.approval_set_id.is_some_and(|set| {
                envelope
                    .expert_approvals
                    .iter()
                    .any(|approval| approval.approval_set_id == set)
            });
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || !identifier(&command.point_id, 512)
                || envelope.resource_key
                    != format!("industrial:{}:{}", command.asset_id, command.point_id)
                || envelope.deployment_stage != Some(command.stage)
                || !matches!(
                    command.stage,
                    DeploymentStage::Simulator
                        | DeploymentStage::DigitalTwin
                        | DeploymentStage::Shadow
                        | DeploymentStage::LimitedWrite
                )
                || command.expected_version != current
                || !command.target_value.is_finite()
                || command.before_digest != envelope.before_digest
                || command.target_digest != envelope.target_digest
                || [
                    &command.prepared_digest,
                    &command.interlock_digest,
                    &command.alarm_digest,
                ]
                .into_iter()
                .any(|value| !digest(value))
                || !approval_matches
                || (limited_write
                    && envelope.physical_supervision.as_ref().is_none_or(|grant| {
                        Some(grant.approval_set_id) != command.approval_set_id
                            || Some(grant.supervision_id) != command.supervision_id
                    }))
        ...
        {
            let command: IndustrialOperationCommand =
                serde_json::from_value(envelope.safe_command.clone())
                    .map_err(|_| DomainProductionError::CommandInvalid)?;
            let current = envelope.binding.resource_version.saturating_sub(1);
            let stage_valid = match envelope.operation.as_str() {
                "industrial.telemetry_read" => matches!(
                    command.stage,
                    DeploymentStage::ReadOnly | DeploymentStage::Shadow
                ),
                "industrial.simulation_run" => matches!(
                    command.stage,
                    DeploymentStage::Simulator | DeploymentStage::DigitalTwin
                ),
                "industrial.operation_stop" => matches!(
                    command.stage,
                    DeploymentStage::Simulator
                        | DeploymentStage::DigitalTwin
                        | DeploymentStage::Shadow
            ...
            let shape_valid = match envelope.operation.as_str() {
                "industrial.telemetry_read" => {
                    command.scenario_digest.is_none()
                        && command.safe_stop_profile_digest.is_none()
                        && command.reason_code.is_none()
                }
                "industrial.simulation_run" => {
                    command.scenario_digest.as_deref().is_some_and(digest)
                        && command.safe_stop_profile_digest.is_none()
                        && command.reason_code.is_none()
                }
                "industrial.operation_stop" => {
            ...
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || envelope.resource_key != format!("industrial:{}", command.asset_id)
                || envelope.deployment_stage != Some(command.stage)
                || command.observed_resource_version != current
                || command.state_digest != envelope.before_digest
                || command.outcome_digest != envelope.target_digest
                || !digest(&command.endpoint_manifest_digest)
                || !(1..=300_000).contains(&command.maximum_age_ms)
                || !stage_valid
                || !shape_valid
            {
                return Err(DomainProductionError::CommandInvalid);
        ...
        {
            let command: EnergyCommand = serde_json::from_value(envelope.safe_command.clone())
                .map_err(|_| DomainProductionError::CommandInvalid)?;
            let supervised_write = command.stage == DeploymentStage::LimitedWrite
                && matches!(
                    envelope.operation.as_str(),
                    "energy.dispatch_commit" | "energy.fallback_activate"
                );
            ...
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || envelope.resource_key != format!("energy:{}", command.asset_id)
                || envelope.deployment_stage != Some(command.stage)
                || !matches!(command.algorithm.as_str(), "MPC" | "RL" | "CBF" | "RULE")
                || [
                    &command.algorithm_manifest_digest,
                    &command.constraint_digest,
                    &command.forecast_digest,
                    &command.fallback_controller_digest,
                ]
                .into_iter()
                .any(|value| !digest(value))
                || command.setpoints_kw.is_empty()
                || command.setpoints_kw.len() > 10_000
                || command.setpoints_kw.iter().any(|value| !value.is_finite())
                || !command.confidence.is_finite()
                || !(0.0..=1.0).contains(&command.confidence)
                || !command.requires_shadow_validation
                || (command.out_of_distribution || command.confidence < 0.8)
                    && command
                        .fallback_reason
                        .as_deref()
                        .is_none_or(|value| !identifier(value, 256))
                || (approval_required && !approval_matches)
                || (!approval_required && command.approval_set_id.is_some())
                || (supervised_write
                    && envelope.physical_supervision.as_ref().is_none_or(|grant| {
                        Some(grant.approval_set_id) != command.approval_set_id
                            || Some(grant.supervision_id) != command.supervision_id
        ...
        DomainKind::Energy
            if matches!(
                envelope.operation.as_str(),
                "energy.telemetry_read" | "energy.forecast_run"
            ) =>
        {
            let command: EnergyObservationCommand =
                serde_json::from_value(envelope.safe_command.clone())
                    .map_err(|_| DomainProductionError::CommandInvalid)?;
            let forecast_shape = if envelope.operation == "energy.forecast_run" {
                command.model_manifest_digest.as_deref().is_some_and(digest)
                    && command.forecast_valid_until.is_some_and(|until| {
                        until > envelope.canonical_action.requested_at
                            && until <= envelope.canonical_action.requested_at + Duration::days(7)
                    })
            } else {
                command.model_manifest_digest.is_none() && command.forecast_valid_until.is_none()
            };
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || envelope.resource_key != format!("energy:{}", command.asset_id)
                || envelope.deployment_stage != Some(command.stage)
                ...
                || command.observed_resource_version
                    != envelope.binding.resource_version.saturating_sub(1)
                || command.telemetry_digest != envelope.before_digest
                || command.output_digest != envelope.target_digest
                || !digest(&command.endpoint_manifest_digest)
                || !forecast_shape
            {
                return Err(DomainProductionError::CommandInvalid);
        ...
        DomainKind::Medical if envelope.operation == "medical.review_request" => {
            let command: MedicalReviewCommand =
                serde_json::from_value(envelope.safe_command.clone())
                    .map_err(|_| DomainProductionError::CommandInvalid)?;
            let reviewer_valid = envelope.expert_approvals.iter().any(|approval| {
                approval.reviewer_subject == command.reviewer_subject
                    && approval.reviewer_role == command.reviewer_role
                    ...
                    && approval.evidence_ref == command.evidence_ref
                    && approval.evidence_digest == command.evidence_digest
            });
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || command.tenant_id != envelope.binding.tenant_id
                || envelope.deployment_stage.is_some()
                || envelope.resource_key != format!("medical-patient:{}", command.patient_ref_hash)
                || command.care_relationship_digest != envelope.before_digest
                || command.recommendation_digest != envelope.target_digest
                || [
                    &command.patient_ref_hash,
                    &command.clinical_evidence_digest,
                    &command.model_manifest_digest,
                    &command.reviewer_qualification_digest,
                    &command.principal_assertion_digest,
                    &command.evidence_digest,
                ]
                .into_iter()
                .any(|value| !digest(value))
                || command.review_id.is_nil()
                || !identifier(&command.reviewer_subject, 512)
                || !matches!(
                    command.risk_level.as_str(),
                    "LOW" | "MEDIUM" | "HIGH" | "CRITICAL"
                )
                || !matches!(
                    command.reviewer_role.as_str(),
                    "CLINICIAN" | "PHYSICIAN" | "PHARMACIST" | "LICENSED_REVIEWER"
                )
                || !matches!(
                ...
                || (matches!(command.risk_level.as_str(), "HIGH" | "CRITICAL") && !reviewer_valid)
            {
                return Err(DomainProductionError::CommandInvalid);
            }
        }
        ...
        {
            let command: MedicalAssistCommand =
                serde_json::from_value(envelope.safe_command.clone())
                    .map_err(|_| DomainProductionError::CommandInvalid)?;
            ...
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                ...
                || command.care_relationship_digest != envelope.before_digest
                || command.output_digest != envelope.target_digest
                || command.requested_fields.is_empty()
                || command.requested_fields.len() > 256
                || command
                    .requested_fields
                    .iter()
                    .any(|field| !safe_field_name(field) || !fields.insert(field.as_str()))
                || !matches!(
                    command.purpose_of_use.as_str(),
                    "TREATMENT" | "OPERATIONS" | "CODING" | "SAFETY_REVIEW"
                )
                || !command.private_deployment
                || [
                    &command.patient_ref_hash,
                    &command.data_policy_decision_digest,
                    &command.residency_decision_digest,
                    &command.model_manifest_digest,
                    &command.clinical_evidence_digest,
                ]
                .into_iter()
        ...
        DomainKind::Sensitive
            if matches!(
                envelope.operation.as_str(),
                "sensitive.human_handoff" | "sensitive.crisis_escalate"
            ) =>
        {
            let command: SensitiveHandoffCommand =
                serde_json::from_value(envelope.safe_command.clone())
                    .map_err(|_| DomainProductionError::CommandInvalid)?;
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || command.tenant_id != envelope.binding.tenant_id
                || envelope.deployment_stage.is_some()
                || envelope.resource_key
                    != format!("sensitive-conversation:{}", command.conversation_hash)
                || command.conversation_hash != envelope.before_digest
                || command.minimum_safe_context_digest != envelope.target_digest
                || !digest(&command.dynamic_region_directory_digest)
                || command.handoff_id.is_nil()
                || !valid_region(&command.region_code)
                || !command.ordinary_agent_paused
                || !matches!(
                    command.risk_level.as_str(),
                    "LOW" | "ELEVATED" | "HIGH" | "IMMINENT"
                )
                || !matches!(
                    command.destination_type.as_str(),
                    "HUMAN_SUPPORT" | "EMERGENCY_SERVICE" | "TRUSTED_CONTACT"
                )
                || !matches!(
                    command.status.as_str(),
                    "REQUESTED" | "ACCEPTED" | "FAILED" | "CLOSED" | "MANUAL_RECOVERY"
                )
            {
                return Err(DomainProductionError::CommandInvalid);
            }
        ...
        {
            let command: SensitiveInteractionCommand =
                serde_json::from_value(envelope.safe_command.clone())
                    .map_err(|_| DomainProductionError::CommandInvalid)?;
            ...
            let risk_valid = if mentor {
                matches!(command.risk_level.as_str(), "ELEVATED" | "HIGH")
            } else {
                matches!(command.risk_level.as_str(), "LOW" | "ELEVATED")
            };
            if command.schema_version != DOMAIN_PACKS_SCHEMA_VERSION
                || command.tenant_id != envelope.binding.tenant_id
                || envelope.deployment_stage.is_some()
                ...
                || command.minimum_safe_context_digest != envelope.target_digest
                || [
                    &command.consent_digest,
                    &command.relationship_boundary_digest,
                    &command.source_snapshot_digest,
                ]
                .into_iter()
                .any(|value| !digest(value))
                || command.citation_digests.is_empty()
                || command.citation_digests.len() > 64
                || command
                    .citation_digests
                    .iter()
                    .any(|value| !digest(value) || !citations.insert(value.as_str()))
                || !risk_valid
...
fn authorize_coding(
    ...
    if lower.contains("/../")
        ...
        || matches!(
            envelope.operation.as_str(),
            "shell" | "deploy.production" | "branch.write.main" | "branch.write.master"
        )
    {
...
fn authorize_physical(
    envelope: &DomainExecutionEnvelope,
    now: DateTime<Utc>,
) -> Result<(), DomainProductionError> {
    let write = envelope.operation.ends_with("commit")
        || envelope.operation.ends_with("restore")
        || envelope.operation.ends_with("activate");
    if !write {
        return Ok(());
    ...
    let stage = envelope
        .deployment_stage
        .ok_or(DomainProductionError::StageDenied)?;
    if !matches!(
        stage,
        DeploymentStage::Simulator
            | DeploymentStage::DigitalTwin
        ...
        return Err(DomainProductionError::StageDenied);
    }
    if stage != DeploymentStage::LimitedWrite {
        return Ok(());
    }
    let approval_set_id = validate_approvals(envelope, now, 2)?;
    let supervision = envelope
        .physical_supervision
        .as_ref()
        .ok_or(DomainProductionError::SupervisionRequired)?;
    if supervision.domain != envelope.domain
        || supervision.stage != DeploymentStage::LimitedWrite
        || supervision.approval_set_id != approval_set_id
        || supervision.resource_key != envelope.resource_key
        || supervision.before_digest != envelope.before_digest
        || supervision.target_digest != envelope.target_digest
        || supervision.resource_version != envelope.binding.resource_version
        || supervision.consumed
        || supervision.issued_at > now
        || supervision.expires_at <= now
        || !postgres_timestamp(&supervision.issued_at)
        || !postgres_timestamp(&supervision.expires_at)
        || !identifier(&supervision.supervisor_subject, 512)
        || !digest(&supervision.supervisor_assertion_digest)
        || !reference(&supervision.evidence_ref)
        || !digest(&supervision.evidence_digest)
        || envelope.expert_approvals.iter().any(|approval| {
            approval.approval_set_id == approval_set_id
                && approval.reviewer_subject == supervision.supervisor_subject
        })
    {
        return Err(DomainProductionError::SupervisionRequired);
    }
...
fn authorize_medical(
    envelope: &DomainExecutionEnvelope,
    now: DateTime<Utc>,
) -> Result<(), DomainProductionError> {
    if envelope.operation.contains("diagnos")
        || envelope.operation.contains("prescri")
        || envelope.operation.contains("treatment")
        || envelope.operation.contains("device_control")
    {
        return Err(DomainProductionError::ToolDenied);
    }
    if envelope.operation == "medical.review_request" {
        let _ = validate_approvals(envelope, now, 1)?;
    }
    Ok(())
}
...
fn authorize_sensitive(
    ...
    if envelope.operation == "sensitive.crisis_escalate"
        || envelope.operation == "sensitive.human_handoff"
    {
        let _ = validate_approvals(envelope, now, 1)?;
        if envelope.safe_command.get("ordinary_agent_paused") != Some(&Value::Bool(true))
            || envelope
                .safe_command
                .get("dynamic_region_directory_digest")
...
pub(crate) fn validate_approvals(
    ...
    now: DateTime<Utc>,
    minimum: usize,
) -> Result<Uuid, DomainProductionError> {
    if minimum == 0
        || minimum > 16
        || envelope.expert_approvals.len() < minimum
        || envelope.expert_approvals.len() > 16
    {
    ...
    for approval in &envelope.expert_approvals {
        if approval.approval_id.is_nil()
            || approval.approval_set_id.is_nil()
            || approval.domain != envelope.domain
            || approval.operation != envelope.operation
            || approval.resource_key != envelope.resource_key
            || approval.before_digest != envelope.before_digest
            || approval.target_digest != envelope.target_digest
            || approval.resource_version != envelope.binding.resource_version
            || approval.approved_at > now
            || approval.expires_at <= now
            || !postgres_timestamp(&approval.approved_at)
            || !postgres_timestamp(&approval.expires_at)
            || approval.expires_at <= approval.approved_at
            || !digest(&approval.qualification_digest)
            || !digest(&approval.principal_assertion_digest)
            || !identifier(&approval.reviewer_subject, 512)
            || approval.reviewer_subject == envelope.actor_subject
            ...
            || !sets
                .entry(approval.approval_set_id)
                .or_default()
                .insert(approval.reviewer_subject.as_str())
        {
            return Err(DomainProductionError::ApprovalRequired);
        }
    }
    ...
    sets.into_iter()
        .find(|(_, reviewers)| reviewers.len() >= minimum)
        .map(|(set, _)| set)
        .ok_or(DomainProductionError::ApprovalRequired)
}
...
fn valid_expert_role(domain: DomainKind, role: &str) -> bool {
    matches!(
        ...
        (
            DomainKind::Coding,
            "CODE_REVIEWER" | "SECURITY_REVIEWER" | "REPOSITORY_MAINTAINER"
        ) | (
            DomainKind::Industrial,
            "CONTROL_ENGINEER" | "SAFETY_ENGINEER" | "SITE_OPERATOR"
        ) | (
            DomainKind::Energy,
            "POWER_SYSTEM_ENGINEER" | "ENERGY_OPERATOR" | "SAFETY_ENGINEER"
        ) | (
            DomainKind::Medical,
            "CLINICIAN" | "PHYSICIAN" | "PHARMACIST" | "LICENSED_REVIEWER"
        ) | (
            DomainKind::Sensitive,
            "TRUST_SAFETY_REVIEWER" | "LICENSED_PROFESSIONAL" | "CRISIS_SUPERVISOR" | "MENTOR"
        )
    )
}

fn normalized_tools(
    tools: &[agent_trust_pack_supply_chain::PackToolDefinition],
) -> BTreeMap<String, (EffectClass, bool, Option<String>, String)> {
    tools
        .iter()
        .map(|tool| {
            (
                ...
                (
                    tool.effect_class,
                    tool.approval_required,
                    tool.compensation_ref.clone(),
                    tool.executor_template.clone(),
                ),
            )
        })
        .collect()
...
fn reference(value: &str) -> bool {
    identifier(value, 1024) && !value.contains("..")
}
fn hex_length(value: &str, lengths: &[usize]) -> bool {
...
fn unsafe_repository_path(value: &str) -> bool {
...
fn valid_region(value: &str) -> bool {
    ...
    let country = parts
        .next()
        .is_some_and(|part| part.len() == 2 && part.bytes().all(|byte| byte.is_ascii_uppercase()));
    let subdivision = parts.next();
    country
        && parts.next().is_none()
        && subdivision.is_none_or(|part| {
...
fn json_limits(value: &Value, depth: usize) -> Result<usize, DomainProductionError> {
    if depth > 32 {
        return Err(DomainProductionError::CommandInvalid);
    }
    match value {
        ...
        _ => Err(DomainProductionError::CommandInvalid),
    }
    .and_then(|size| {
        if size <= 1_048_576 {
            Ok(size)
        } else {
            Err(DomainProductionError::CommandInvalid)
        }
    })
}
pub fn safe_digest(value: &impl Serialize) -> Result<String, DomainProductionError> {
...
pub enum DomainProductionError {
    ...
    PackInvalid,
    #[error("DOMAIN_BINDING_INVALID")]
    BindingInvalid,
    #[error("DOMAIN_COMMAND_INVALID")]
    CommandInvalid,
    #[error("DOMAIN_TOOL_DENIED")]
    ToolDenied,
    #[error("DOMAIN_STAGE_DENIED")]
    StageDenied,
    #[error("DOMAIN_APPROVAL_REQUIRED")]
    ApprovalRequired,
    #[error("DOMAIN_PHYSICAL_SUPERVISION_REQUIRED")]
    SupervisionRequired,
    #[error("DOMAIN_ESCALATION_INVALID")]
    EscalationInvalid,
}
