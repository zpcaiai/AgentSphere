...
pub enum DomainEvaluatorConclusion {
    Pass,
    Fail,
    NeedsHuman,
    ManualRecovery,
...
impl DomainReceiptKeyring {
    pub fn from_json(raw: &[u8], now: DateTime<Utc>) -> Result<Self, DomainAuthorityError> {
        ...
        let document: KeyringDocument =
            serde_json::from_slice(raw).map_err(|_| DomainAuthorityError::ConfigurationInvalid)?;
        if document.schema_version != DOMAIN_KEYRING_SCHEMA
        ...
        for entry in document.keys {
            ...
            let key = VerifyingKey::from_bytes(&bytes)
                .map_err(|_| DomainAuthorityError::ConfigurationInvalid)?;
            if entry.usage != "DOMAIN_RUNTIME_RECEIPT"
                || entry.revoked
    ...
    fn verify(
        ...
        let (key, not_before, expires_at) = self
            .keys
            .get(&receipt.key_id)
            .ok_or(DomainAuthorityError::ReceiptInvalid)?;
        if receipt.schema_version != DOMAIN_RECEIPT_SCHEMA
            || receipt.execution_id != envelope.execution_id
            || receipt.domain != envelope.domain
            || receipt.operation != envelope.operation
            || receipt.resource_key != envelope.resource_key
            || receipt.resource_version != envelope.binding.resource_version
            || receipt.action_hash != action_hash
            || receipt.request_digest != request_digest
            || receipt.completed_at < envelope.canonical_action.requested_at
            ...
            || receipt
                ...
                != Some(conclusion_name(receipt.conclusion))
...
fn validate_typed_runtime_payloads(
    receipt: &DomainRuntimeReceipt,
    envelope: &DomainExecutionEnvelope,
    effect: EffectClass,
) -> Result<(), DomainAuthorityError> {
    let effect_receipt: TypedDomainEffectReceipt =
        serde_json::from_value(receipt.effect_receipt.clone())
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
    let tool = envelope
        .pack_manifest
        .tools
        .iter()
        .find(|tool| tool.tool_id == envelope.operation)
        .ok_or(DomainAuthorityError::ReceiptInvalid)?;
    let external_pair = match (
        &effect_receipt.external_receipt_ref,
        &effect_receipt.external_receipt_digest,
    ) {
        (Some(reference_value), Some(digest_value))
            if reference(reference_value, 2048) && digest(digest_value) =>
        {
            true
        }
        (None, None) => false,
    ...
    if effect_receipt.schema_version != DOMAIN_EFFECT_RECEIPT_SCHEMA
        || effect_receipt.tenant_id != envelope.binding.tenant_id
        || effect_receipt.execution_id != envelope.execution_id
        || effect_receipt.tool_id != envelope.operation
        || effect_receipt.operation != envelope.operation
        || effect_receipt.resource_key != envelope.resource_key
        || effect_receipt.resource_version != envelope.binding.resource_version
        || effect_receipt.before_digest != envelope.before_digest
        || effect_receipt.target_digest != envelope.target_digest
        || !identifier(&effect_receipt.executor_identity, 512)
        || !reference(&effect_receipt.executor_receipt_ref, 2048)
        || !digest(&effect_receipt.executor_receipt_digest)
        || effect_receipt.compensation_ref.as_deref() != tool.compensation_ref.as_deref()
        || effect_receipt.started_at < envelope.canonical_action.requested_at
        || effect_receipt.completed_at < effect_receipt.started_at
        || effect_receipt.completed_at > receipt.completed_at
        || (effect == EffectClass::Pure && (effect_receipt.status != "OBSERVED" || external_pair))
        || (effect != EffectClass::Pure
            && (!matches!(effect_receipt.status.as_str(), "APPLIED" | "COMPENSATED")
                || !external_pair))
    {
        return Err(DomainAuthorityError::ReceiptInvalid);
    }

    let evaluator: TypedDomainEvaluatorResult =
        serde_json::from_value(receipt.evaluator_result.clone())
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
    let expected_checks = required_evaluator_checks(envelope.domain);
    let actual_checks = evaluator
        .checks
        .keys()
        .map(String::as_str)
        .collect::<BTreeSet<_>>();
    let reasons_valid = evaluator.reason_codes.len() <= 64
        && evaluator
            .reason_codes
            .iter()
            .all(|reason| identifier(reason, 128));
    let all_pass = evaluator.checks.values().all(|passed| *passed);
    if evaluator.schema_version != DOMAIN_EVALUATOR_RESULT_SCHEMA
        || evaluator.domain != envelope.domain
        || evaluator.operation != envelope.operation
        || evaluator.resource_key != envelope.resource_key
        || evaluator.before_digest != envelope.before_digest
        || evaluator.target_digest != envelope.target_digest
        || evaluator.conclusion != receipt.conclusion
        || actual_checks != expected_checks
        || !reasons_valid
        || evaluator.evaluated_at < effect_receipt.completed_at
        || evaluator.evaluated_at > receipt.completed_at
        || (evaluator.conclusion == DomainEvaluatorConclusion::Pass
            && (!all_pass || !evaluator.reason_codes.is_empty()))
        || (evaluator.conclusion != DomainEvaluatorConclusion::Pass
            && (all_pass || evaluator.reason_codes.is_empty()))
    {
        return Err(DomainAuthorityError::ReceiptInvalid);
...
fn required_evaluator_checks(domain: DomainKind) -> BTreeSet<&'static str> {
    match domain {
        DomainKind::Coding => BTreeSet::from([
            "build_test_gate",
            "command_template",
            "dependency_lock",
            "path_scope",
        ...
        DomainKind::Industrial => BTreeSet::from([
            "alarm_clear",
            "interlock_healthy",
            "quality_good",
            "rate_limit",
        ...
        DomainKind::Energy => BTreeSet::from([
            "fallback_ready",
            "forecast_valid",
            "hard_constraints",
            "not_ood",
        ...
        DomainKind::Medical => BTreeSet::from([
            "clinical_evidence",
            "minimum_necessary",
            "patient_match",
            "privacy_boundary",
        ...
        DomainKind::Sensitive => BTreeSet::from([
            "citation_integrity",
            "consent",
            "escalation",
            "manipulation_absent",
...
pub trait DomainRuntimePort: Send + Sync {
    async fn execute(
        ...
        request_digest: &str,
        action_hash: &str,
    ) -> Result<DomainRuntimeReceipt, DomainAuthorityError>;
    async fn deliver_evidence(
        ...
        payload: &Value,
        payload_digest: &str,
    ) -> Result<AuthorityEvidenceDelivery, DomainAuthorityError>;
    async fn ready(&self) -> bool;
}

#[derive(Clone)]
pub struct PostgresDomainRuntimeStore {
    pool: PgPool,
}
...
impl PostgresDomainRuntimeStore {
    ...
    async fn begin_tenant(
        &self,
        tenant: Uuid,
    ) -> Result<Transaction<'_, Postgres>, DomainAuthorityError> {
        let mut tx = self
            .pool
            .begin()
            .await
            .map_err(|_| DomainAuthorityError::DependencyUnavailable)?;
    ...
    pub async fn ready(&self) -> bool {
        sqlx::query_scalar::<_, i32>(
            "SELECT 1 FROM public.domain_pack_executions WHERE false UNION ALL SELECT 1 LIMIT 1",
        )
    ...
    async fn verifier(
        ...
        tenant: Uuid,
        envelope: &DomainExecutionEnvelope,
    ) -> Result<ArtifactVerifier, DomainAuthorityError> {
        ...
        if !active {
            return Err(DomainAuthorityError::PackInactive);
        }
        ...
        if row.get::<String, _>("algorithm") != "ED25519"
            ...
            || row.get::<DateTime<Utc>, _>("valid_from") > signed_at
            || row.get::<DateTime<Utc>, _>("valid_until") <= signed_at
        {
            return Err(DomainAuthorityError::PackInactive);
        }
        ...
        let key =
            VerifyingKey::from_bytes(&bytes).map_err(|_| DomainAuthorityError::PackInactive)?;
        let verifier = ArtifactVerifier::default();
        verifier.authorize_publisher(
            envelope.pack_manifest.signature.key_id.clone(),
            envelope.pack_manifest.publisher_identity.clone(),
            key,
        );
        tx.commit()
            .await
    ...
    async fn prepare_and_claim(
        ...
        instance_id: Uuid,
        lease_seconds: i64,
    ) -> Result<Option<DomainExecutionResult>, DomainAuthorityError> {
        let tenant = envelope.binding.tenant_id;
        let mut tx = self.begin_tenant(tenant).await?;
        let canonical = serde_json::to_value(&envelope.canonical_action)
            .map_err(|_| DomainAuthorityError::RequestInvalid)?;
        let task_id = Uuid::parse_str(&envelope.canonical_action.task_id.0)
            .map_err(|_| DomainAuthorityError::RequestInvalid)?;
        let input_digest =
            safe_digest(&envelope.safe_command).map_err(DomainAuthorityError::from)?;
        ...
        if row.get::<Uuid, _>("execution_id") != envelope.execution_id
            || row.get::<String, _>("request_digest") != request_digest
            || row.get::<String, _>("action_hash") != action_hash
            || row.get::<String, _>("resource_key") != envelope.resource_key
            || row.get::<i64, _>("resource_version")
                != i64::try_from(envelope.binding.resource_version)
                    .map_err(|_| DomainAuthorityError::RequestInvalid)?
        {
            return Err(DomainAuthorityError::IdempotencyConflict);
        }
        if row.get::<String, _>("state") != "PREPARED" {
        ...
        let tool = envelope
            ...
            .tools
            .iter()
            .find(|tool| tool.tool_id == envelope.canonical_action.tool.tool_id.0)
            .ok_or(DomainAuthorityError::RequestInvalid)?;
        let minimum = if envelope.physical_supervision.is_some() {
            2usize
        } else if tool.approval_required {
            1usize
        } else {
            0usize
        };
        if minimum > 0 {
            let approval_set = validate_approvals(envelope, Utc::now(), minimum)
                .map_err(|_| DomainAuthorityError::ApprovalInvalid)?;
            if envelope
                .physical_supervision
                .as_ref()
                .is_some_and(|grant| grant.approval_set_id != approval_set)
            {
                return Err(DomainAuthorityError::ApprovalInvalid);
            }
            let approval_ids = envelope
                .expert_approvals
                .iter()
                .filter(|approval| approval.approval_set_id == approval_set)
                .map(|approval| approval.approval_id)
                .collect::<Vec<_>>();
            let supervisor = envelope
                .physical_supervision
                .as_ref()
                .map(|grant| grant.supervisor_subject.as_str());
            ...
            if rows.len() != approval_ids.len() || rows.len() < minimum {
                return Err(DomainAuthorityError::ApprovalInvalid);
            }
            for approval in &envelope.expert_approvals {
                let row = rows
                    .iter()
                    .find(|row| row.get::<Uuid, _>("approval_id") == approval.approval_id)
                    .ok_or(DomainAuthorityError::ApprovalInvalid)?;
                if row.get::<Uuid, _>("approval_set_id") != approval_set
                    || row.get::<String, _>("domain") != domain_name(envelope.domain)
                    || row.get::<String, _>("operation") != envelope.operation
                    || row.get::<String, _>("resource_key") != envelope.resource_key
                    || row.get::<String, _>("before_digest") != envelope.before_digest
                    || row.get::<String, _>("target_digest") != envelope.target_digest
                    || row.get::<i64, _>("resource_version")
                        != i64::try_from(envelope.binding.resource_version)
                            .map_err(|_| DomainAuthorityError::RequestInvalid)?
                    || row.get::<String, _>("decision") != "APPROVED"
                    || row.get::<String, _>("reviewer_subject") != approval.reviewer_subject
                    || row.get::<String, _>("reviewer_role") != approval.reviewer_role
                    || row.get::<String, _>("reviewer_qualification_digest")
                        != approval.qualification_digest
                    || row.get::<String, _>("principal_assertion_digest")
                        != approval.principal_assertion_digest
                    || row.get::<String, _>("evidence_ref") != approval.evidence_ref
                    || row.get::<String, _>("evidence_digest") != approval.evidence_digest
                    || row.get::<DateTime<Utc>, _>("approved_at") != approval.approved_at
                    || row.get::<DateTime<Utc>, _>("expires_at") != approval.expires_at
                    || row.get::<DateTime<Utc>, _>("approved_at") > Utc::now()
                    || row.get::<DateTime<Utc>, _>("expires_at") <= Utc::now()
                    || supervisor == Some(approval.reviewer_subject.as_str())
                {
                    return Err(DomainAuthorityError::ApprovalInvalid);
                }
            }
        ...
        if let Some(grant) = &envelope.physical_supervision {
            ...
            if affected != 1 {
                return Err(DomainAuthorityError::SupervisionInvalid);
            }
        }
        ...
        if affected != 1 {
            return Err(DomainAuthorityError::StateConflict);
        }
        tx.commit()
            .await
            .map_err(|_| DomainAuthorityError::DependencyUnavailable)?;
    ...
    async fn commit_receipt(
        ...
        envelope: &DomainExecutionEnvelope,
        receipt: &DomainRuntimeReceipt,
    ) -> Result<DomainExecutionResult, DomainAuthorityError> {
        let tenant = envelope.binding.tenant_id;
        let mut tx = self.begin_tenant(tenant).await?;
        ...
        let receipt_value = serde_json::to_value(&receipt.effect_receipt)
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
        let evaluator_value = serde_json::to_value(&receipt.evaluator_result)
            .map_err(|_| DomainAuthorityError::ReceiptInvalid)?;
        ...
        let (terminal, stable) = match receipt.conclusion {
            DomainEvaluatorConclusion::Pass | DomainEvaluatorConclusion::Fail => {
                ("SUCCEEDED", None)
            }
            DomainEvaluatorConclusion::NeedsHuman => {
                ("NEEDS_HUMAN", Some("DOMAIN_EVALUATOR_NEEDS_HUMAN"))
            }
            DomainEvaluatorConclusion::ManualRecovery => {
                ("MANUAL_RECOVERY", Some("DOMAIN_EVALUATOR_MANUAL_RECOVERY"))
            }
        };
        ...
        tx.commit()
            .await
            .map_err(|_| DomainAuthorityError::OutcomeUnknown)?;
        Ok(DomainExecutionResult {
            schema_version: DOMAIN_RESULT_SCHEMA.into(),
            execution_id: envelope.execution_id,
            state: terminal.into(),
            domain: envelope.domain,
            resource_key: envelope.resource_key.clone(),
            resource_version: envelope.binding.resource_version,
            effect_receipt_digest: Some(receipt.effect_receipt_digest.clone()),
            evaluator_result_digest: Some(receipt.evaluator_result_digest.clone()),
            evaluator_conclusion: Some(receipt.conclusion),
            evidence_ref: Some(evidence_ref),
            evidence_digest: Some(payload_digest),
            stable_error: stable.map(str::to_string),
        })
    }
    ...
    async fn finish_uncertain(
        ...
        if !matches!(state, "FAILED" | "UNKNOWN") || !identifier(error, 128) {
            return Err(DomainAuthorityError::RequestInvalid);
        }
    ...
    pub async fn recover_expired(
        ...
        tenant: Uuid,
        limit: i64,
    ) -> Result<u64, DomainAuthorityError> {
        ...
        for row in &rows {
            sqlx::query("UPDATE public.domain_pack_executions SET state='UNKNOWN',stable_error='DOMAIN_RUNTIME_LEASE_EXPIRED',updated_at=now() WHERE tenant_id=$1 AND execution_id=$2 AND state IN ('EXECUTING','VERIFYING')")
            .bind(tenant).bind(row.get::<Uuid,_>("execution_id")).execute(&mut *tx).await.map_err(|_|DomainAuthorityError::OutcomeUnknown)?;
        }
        tx.commit()
            .await
            .map_err(|_| DomainAuthorityError::OutcomeUnknown)?;
        u64::try_from(rows.len()).map_err(|_| DomainAuthorityError::DependencyUnavailable)
    }

    pub async fn authoritative_state(
        &self,
        tenant: Uuid,
        limit: i64,
        cursor: Option<&str>,
    ) -> Result<AuthoritativeDomainExecutionPage, DomainAuthorityError> {
        if !(1..=200).contains(&limit) {
            return Err(DomainAuthorityError::RequestInvalid);
        }
        let decoded = cursor.map(decode_cursor).transpose()?;
        let after_time = decoded.as_ref().map(|value| value.0);
        let after_id = decoded.as_ref().map(|value| value.1);
        let mut tx = self.begin_tenant(tenant).await?;
        ...
        let items = rows
            .into_iter()
            .take(usize::try_from(limit).map_err(|_| DomainAuthorityError::RequestInvalid)?)
            .map(authoritative_from_row)
            .collect::<Result<Vec<_>, _>>()?;
        let next_cursor = if has_more {
            items
                .last()
                .map(|item| encode_cursor(item.created_at, item.execution_id))
                .transpose()?
        } else {
            None
        };
        ...
        Ok(AuthoritativeDomainExecutionPage {
            schema_version: DOMAIN_STATE_SCHEMA.into(),
            tenant_id: tenant,
            authoritative: true,
            data_digest,
            items,
...
#[derive(Clone)]
pub struct DomainRuntimeAuthority {
    store: PostgresDomainRuntimeStore,
    runtime: Arc<dyn DomainRuntimePort>,
    keyring: DomainReceiptKeyring,
    instance_id: Uuid,
    lease_seconds: i64,
}
...
impl DomainRuntimeAuthority {
    pub fn new(
        ...
        instance_id: Uuid,
        lease_seconds: i64,
    ) -> Result<Self, DomainAuthorityError> {
        if instance_id.is_nil() || !(15..=300).contains(&lease_seconds) {
            return Err(DomainAuthorityError::ConfigurationInvalid);
        }
        Ok(Self {
            store,
    ...
    pub async fn execute(
        ...
        let verifier = self
            .store
            .verifier(envelope.binding.tenant_id, &envelope)
            .await?;
        let action_hash = ProductionDomainPackContract::authorize(&envelope, &verifier, Utc::now())
            .map_err(DomainAuthorityError::from)?;
        ...
        let receipt = match self
            .runtime
            .execute(&envelope, &request_digest, &action_hash)
            .await
        {
            Ok(value) => value,
            Err(error) => {
                let unknown = effect != EffectClass::Pure;
                self.store
                    .finish_uncertain(
                        &envelope,
                        if unknown { "UNKNOWN" } else { "FAILED" },
                        if unknown {
                            "DOMAIN_RUNTIME_EXTERNAL_OUTCOME_UNKNOWN"
                        } else {
                            error.code()
                        },
                    )
                ...
                return Err(if unknown {
                    DomainAuthorityError::OutcomeUnknown
                } else {
                    error
                });
            }
        ...
        if let Err(error) = self.keyring.verify(
            ...
            self.store
                .finish_uncertain(&envelope, "UNKNOWN", "DOMAIN_RUNTIME_RECEIPT_INVALID")
                .await?;
            return Err(error);
        }
            ...
            Err(_) => {
                ...
                let _ = self
                    ...
                    .finish_uncertain(
                        &envelope,
                        "UNKNOWN",
                        "DOMAIN_RUNTIME_COMMIT_OUTCOME_UNKNOWN",
                    )
                    .await;
                return Err(DomainAuthorityError::OutcomeUnknown);
            }
        };
        let _ = self.flush_evidence(envelope.binding.tenant_id, 32).await;
        Ok(result)
    }
    pub async fn authoritative_state(
        &self,
        ...
        self.store.authoritative_state(tenant, limit, cursor).await
    }
    pub async fn recover_expired(
        &self,
        tenant: Uuid,
    ...
    pub async fn flush_evidence(
        ...
        let mut tx = self.store.begin_tenant(tenant).await?;
        ...
        for row in rows {
            ...
            let idempotency: String = row.get("idempotency_key");
            let payload: Value = row.get("payload");
            let digest_value: String = row.get("payload_digest");
            let receipt = self
                .runtime
                .deliver_evidence(tenant, &idempotency, &payload, &digest_value)
                .await?;
            if !digest(&digest_value)
                || !digest(&receipt.evidence_digest)
                || !reference(&receipt.evidence_ref, 2048)
            {
                return Err(DomainAuthorityError::ReceiptInvalid);
            }
...
fn result_from_row(
    ...
    let evaluator = row.get::<Option<Value>, _>("evaluator_result");
    let conclusion = evaluator
        .as_ref()
    ...
    Ok(DomainExecutionResult {
        ...
        state: row.get("state"),
        domain: parse_domain(&row.get::<String, _>("domain"))?,
        resource_key: row.get("resource_key"),
        resource_version: u64::try_from(row.get::<i64, _>("resource_version"))
            .map_err(|_| DomainAuthorityError::StateConflict)?,
        effect_receipt_digest: row.get("effect_receipt_digest"),
        evaluator_result_digest: row.get("evaluator_result_digest"),
        evaluator_conclusion: conclusion,
        evidence_ref: row.get("evidence_ref"),
        evidence_digest: row.get("evidence_digest"),
        stable_error: row.get("stable_error"),
    })
}
fn authoritative_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<AuthoritativeDomainExecution, DomainAuthorityError> {
    Ok(AuthoritativeDomainExecution {
        execution_id: row.get("execution_id"),
        command_id: row.get("command_id"),
        domain: parse_domain(&row.get::<String, _>("domain"))?,
        pack_id: row.get("pack_id"),
        pack_version: row.get("pack_version"),
        pack_manifest_digest: row.get("pack_manifest_digest"),
        ...
        effect_receipt_digest: row.get("effect_receipt_digest"),
        evaluator_result_digest: row.get("evaluator_result_digest"),
        evaluator_conclusion: row.get("evaluator_conclusion"),
        evidence_ref: row.get("evidence_ref"),
        evidence_digest: row.get("evidence_digest"),
        stable_error: row.get("stable_error"),
...
fn domain_name(value: DomainKind) -> &'static str {
    match value {
        DomainKind::Coding => "CODING",
        DomainKind::Industrial => "INDUSTRIAL",
        DomainKind::Energy => "ENERGY",
        DomainKind::Medical => "MEDICAL",
        DomainKind::Sensitive => "SENSITIVE",
    }
}
fn parse_domain(value: &str) -> Result<DomainKind, DomainAuthorityError> {
    match value {
        "CODING" => Ok(DomainKind::Coding),
        "INDUSTRIAL" => Ok(DomainKind::Industrial),
        "ENERGY" => Ok(DomainKind::Energy),
        "MEDICAL" => Ok(DomainKind::Medical),
        "SENSITIVE" => Ok(DomainKind::Sensitive),
        _ => Err(DomainAuthorityError::StateConflict),
    }
}
fn effect_name(value: EffectClass) -> &'static str {
    match value {
...
fn parse_conclusion(value: &str) -> Option<DomainEvaluatorConclusion> {
    match value {
        "PASS" => Some(DomainEvaluatorConclusion::Pass),
        "FAIL" => Some(DomainEvaluatorConclusion::Fail),
        "NEEDS_HUMAN" => Some(DomainEvaluatorConclusion::NeedsHuman),
        "MANUAL_RECOVERY" => Some(DomainEvaluatorConclusion::ManualRecovery),
        _ => None,
    }
}
fn conclusion_name(value: DomainEvaluatorConclusion) -> &'static str {
    match value {
        DomainEvaluatorConclusion::Pass => "PASS",
        DomainEvaluatorConclusion::Fail => "FAIL",
        DomainEvaluatorConclusion::NeedsHuman => "NEEDS_HUMAN",
        DomainEvaluatorConclusion::ManualRecovery => "MANUAL_RECOVERY",
    }
}
...
fn decode_cursor(value: &str) -> Result<(DateTime<Utc>, Uuid), DomainAuthorityError> {
    ...
    let fields: Vec<String> =
        serde_json::from_slice(&raw).map_err(|_| DomainAuthorityError::RequestInvalid)?;
    if fields.len() != 2 {
        return Err(DomainAuthorityError::RequestInvalid);
    }
    let at = DateTime::parse_from_rfc3339(&fields[0])
        ...
        .map_err(|_| DomainAuthorityError::RequestInvalid)?;
    let id = Uuid::parse_str(&fields[1]).map_err(|_| DomainAuthorityError::RequestInvalid)?;
    Ok((at, id))
}
fn encode_cursor(at: DateTime<Utc>, id: Uuid) -> Result<String, DomainAuthorityError> {
    serde_jcs::to_vec(&[
...
impl From<DomainProductionError> for DomainAuthorityError {
    fn from(value: DomainProductionError) -> Self {
...
pub enum DomainAuthorityError {
    #[error("DOMAIN_RUNTIME_CONTRACT_DENIED:{0}")]
    Contract(String),
    #[error("DOMAIN_RUNTIME_REQUEST_INVALID")]
    RequestInvalid,
    #[error("DOMAIN_RUNTIME_PACK_INACTIVE")]
    PackInactive,
    #[error("DOMAIN_RUNTIME_APPROVAL_INVALID")]
    ApprovalInvalid,
    #[error("DOMAIN_RUNTIME_SUPERVISION_INVALID")]
    SupervisionInvalid,
    #[error("DOMAIN_RUNTIME_IDEMPOTENCY_CONFLICT")]
    IdempotencyConflict,
    #[error("DOMAIN_RUNTIME_STATE_CONFLICT")]
    StateConflict,
    #[error("DOMAIN_RUNTIME_RECEIPT_INVALID")]
    ReceiptInvalid,
    #[error("DOMAIN_RUNTIME_OUTCOME_UNKNOWN")]
    OutcomeUnknown,
    #[error("DOMAIN_RUNTIME_DEPENDENCY_UNAVAILABLE")]
    DependencyUnavailable,
    #[error("DOMAIN_RUNTIME_PRINCIPAL_DENIED")]
    PrincipalDenied,
    #[error("DOMAIN_RUNTIME_CONFIGURATION_INVALID")]
    ConfigurationInvalid,
}
impl DomainAuthorityError {
    pub fn code(&self) -> &str {
        match self {
            Self::Contract(_) => "DOMAIN_RUNTIME_CONTRACT_DENIED",
            Self::RequestInvalid => "DOMAIN_RUNTIME_REQUEST_INVALID",
            Self::PackInactive => "DOMAIN_RUNTIME_PACK_INACTIVE",
            Self::ApprovalInvalid => "DOMAIN_RUNTIME_APPROVAL_INVALID",
            Self::SupervisionInvalid => "DOMAIN_RUNTIME_SUPERVISION_INVALID",
            Self::IdempotencyConflict => "DOMAIN_RUNTIME_IDEMPOTENCY_CONFLICT",
            Self::StateConflict => "DOMAIN_RUNTIME_STATE_CONFLICT",
            Self::ReceiptInvalid => "DOMAIN_RUNTIME_RECEIPT_INVALID",
            Self::OutcomeUnknown => "DOMAIN_RUNTIME_OUTCOME_UNKNOWN",
            Self::DependencyUnavailable => "DOMAIN_RUNTIME_DEPENDENCY_UNAVAILABLE",
            Self::PrincipalDenied => "DOMAIN_RUNTIME_PRINCIPAL_DENIED",
            Self::ConfigurationInvalid => "DOMAIN_RUNTIME_CONFIGURATION_INVALID",
        }
    }
}
