...
use crate::{ArtifactManifest, ArtifactVerifier, DomainPackManifest, PackError, PackSdk, PermissionDiff};
...
impl SupplyReceiptKeyring {
    ...
    fn verify(
        ...
        let required_receipts_present=match request.command.operation {
            SupplyOperation::Publish=>receipt.repository_receipt_digest.is_some()
                &&receipt.signature_receipt_digest.is_some()&&receipt.sbom_receipt_digest.is_some()
                &&receipt.vulnerability_receipt_digest.is_some()&&receipt.license_receipt_digest.is_some(),
            SupplyOperation::Validate=>receipt.sandbox_receipt_digest.is_some()
                &&receipt.vulnerability_receipt_digest.is_some()&&receipt.license_receipt_digest.is_some(),
            SupplyOperation::Approve=>receipt.external_effect_count==0
                &&receipt.repository_receipt_digest.is_none()&&receipt.signature_receipt_digest.is_none()
                &&receipt.sbom_receipt_digest.is_none()&&receipt.vulnerability_receipt_digest.is_none()
                &&receipt.license_receipt_digest.is_none()&&receipt.sandbox_receipt_digest.is_none()
                &&receipt.revocation_receipt_digest.is_none()&&receipt.installation_receipt_digest.is_none()
                &&receipt.reconciliation_receipt_digest.is_none(),
            SupplyOperation::Activate|SupplyOperation::Rollback=>receipt.installation_receipt_digest.is_some(),
            SupplyOperation::Revoke|SupplyOperation::Quarantine=>receipt.revocation_receipt_digest.is_some(),
            SupplyOperation::Recover=>receipt.reconciliation_receipt_digest.is_some(),
        };
        if !required_receipts_present{return Err(SupplyAuthorityError::ReceiptInvalid);}
        for digest_value in [
...
impl SupplyExecutionRequest {
    ...
    pub fn validate(&self, now: DateTime<Utc>) -> Result<String, SupplyAuthorityError> {
        ...
        let payload_resource=validate_supply_payload(self.command.operation,&self.command.safe_payload,self.command.requested_at)?;
        if self.schema_version != SUPPLY_EXECUTION_SCHEMA
            ...
            || self.canonical_action.intent.operation != self.command.operation.as_str().to_ascii_lowercase()
            ...
            || self.binding.resource_version != self.command.expected_resource_version.saturating_add(1)
        ...
        if self.canonical_action.payload.type_id != "supply-chain.mutation.v1"
            ...
            || self.canonical_action.extensions.get("x-policy-decision-digest")
                != Some(&Value::String(self.binding.policy_decision_digest.clone()))
            || self.canonical_action.extensions.get("x-authorization-evidence-digest")
                != Some(&Value::String(self.binding.authorization_evidence_digest.clone()))
            || self.canonical_action.extensions.get("x-ledger-event-digest")
                != Some(&Value::String(self.binding.ledger_event_digest.clone()))
            || self.canonical_action.extensions.get("x-execution-fence-digest")
                != Some(&Value::String(self.binding.fence_digest.clone()))
            || self.canonical_action.extensions.get("x-supply-command-digest")
                != Some(&Value::String(canonical_digest(&self.command.safe_payload)?))
        {
...
pub trait PackRegistry: Send + Sync {
    async fn publish(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
    async fn approve(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
    async fn activate(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
    async fn revoke(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
}

#[async_trait]
pub trait PackInstaller: Send + Sync {
    async fn install(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
    async fn rollback(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
}

#[async_trait]
pub trait RevocationService: Send + Sync {
    async fn quarantine(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
    async fn revoke_release(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError>;
}

pub struct CompatibilityResolver;

impl CompatibilityResolver {
    pub fn check(required: &BTreeSet<String>, provided: &BTreeSet<String>) -> Result<(), SupplyAuthorityError> {
...
impl PackValidator {
    pub fn validate(
        ...
        if manifest.artifact_refs.len()!=artifacts.len() { return Err(SupplyAuthorityError::PackInvalid); }
        for reference in &manifest.artifact_refs {
            let artifact=artifacts.get(reference).ok_or(SupplyAuthorityError::PackInvalid)?;
            verifier.verify_artifact(artifact).map_err(map_pack_error)?;
            if reference!=&format!("artifact:sha256:{}",artifact.digest) { return Err(SupplyAuthorityError::PackInvalid); }
        }
        Ok(())
    }
}
...
impl SupplyChainGate {
    pub fn admit(
        ...
        if !conformance_passed||!behavior_passed
            ||(!manifest.permissions.approval_scopes.is_empty()&&!permission_expansion_approved)
        {return Err(SupplyAuthorityError::ApprovalDenied);} Ok(())
    }
}
...
impl PostgresSupplyChainStore {
    ...
    pub async fn authoritative_releases(
        ...
        if !(1..=200).contains(&limit){return Err(SupplyAuthorityError::RequestInvalid);}
        ...
        tx.commit().await.map_err(|_|SupplyAuthorityError::DependencyUnavailable)?;
        let has_more=i64::try_from(rows.len()).is_ok_and(|count|count>limit);
        let mut items=rows.into_iter().take(usize::try_from(limit).map_err(|_|SupplyAuthorityError::RequestInvalid)?)
            .map(authoritative_release_from_row).collect::<Result<Vec<_>,_>>()?;
        let next_cursor=if has_more{items.last().map(|item|encode_release_cursor(&item.pack_id,&item.version)).transpose()?}else{None};
        ...
        Ok(AuthoritativeReleasePage{schema_version:SUPPLY_RELEASES_SCHEMA.into(),tenant_id:tenant,authoritative:true,data_digest:digest_value,items:std::mem::take(&mut items),next_cursor})
    }
    ...
    async fn prepare_and_claim(
        ...
        if row.get::<Uuid, _>("command_id") != request.command.command_id
            ...
            || row.get::<i64, _>("resource_version")
                != i64::try_from(request.binding.resource_version).map_err(|_| SupplyAuthorityError::RequestInvalid)?
        {
        ...
        .await
        .map_err(|_| SupplyAuthorityError::StateConflict)?.rows_affected();
        if affected!=1{return Err(SupplyAuthorityError::StateConflict);}
        tx.commit().await.map_err(|_| SupplyAuthorityError::DependencyUnavailable)?;
        Ok(None)
    }
    ...
    async fn verify_manifest_signature(
        ...
        let key_subject=format!("{}#{}",manifest.publisher_identity,manifest.signature.key_id);
        ...
        ).bind(tenant).bind(publisher_subject).bind(key_subject).fetch_one(&mut **tx).await
            .map_err(|_|SupplyAuthorityError::DependencyUnavailable)?;
        if revoked{return Err(SupplyAuthorityError::PublisherDenied);}
        let key = VerifyingKey::from_bytes(&bytes).map_err(|_| SupplyAuthorityError::PublisherDenied)?;
    ...
    async fn preflight_external(
        ...
        let body=request.command.safe_payload.as_object().ok_or(SupplyAuthorityError::RequestInvalid)?;
        let expected=i64::try_from(request.command.expected_resource_version).map_err(|_|SupplyAuthorityError::RequestInvalid)?;
        match request.command.operation{
            SupplyOperation::Publish=>{
                let manifest:DomainPackManifest=serde_json::from_value(required_object_field(&request.command.safe_payload,"manifest")?)
                    .map_err(|_|SupplyAuthorityError::RequestInvalid)?;
                ...
                if exists||expected!=0{return Err(SupplyAuthorityError::StateConflict);}
            }
            SupplyOperation::Validate=>{
                let exists:bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM public.supply_chain_pack_releases WHERE tenant_id=$1 AND pack_id=$2 AND version=$3 AND lifecycle_state='PUBLISHED' AND resource_version=$4)")
                    ...
                    .fetch_one(&mut **tx).await.map_err(|_|SupplyAuthorityError::DependencyUnavailable)?;
                if !exists{return Err(SupplyAuthorityError::StateConflict);}
            }
            SupplyOperation::Activate=>{
                let pack_id=string_field(body,"pack_id",256)?;let version=string_field(body,"version",128)?;
                let environment=string_field(body,"environment",64)?;let manifest_digest=string_field(body,"manifest_digest",64)?;
                ...
                if !releasable||installed.is_some_and(|version|version!=expected)||installed.is_none()&&expected!=0{return Err(SupplyAuthorityError::ApprovalDenied);}
            }
            SupplyOperation::Rollback=>{
                let pack_id=string_field(body,"pack_id",256)?;let environment=string_field(body,"environment",64)?;
                ...
                if !recoverable{return Err(SupplyAuthorityError::RecoveryDenied);}
            }
            SupplyOperation::Recover=>{
                ...
                if state.as_deref()!=Some("UNKNOWN"){return Err(SupplyAuthorityError::RecoveryDenied);}
            }
    ...
    async fn commit_success(
        ...
        let receipt_value = serde_json::to_value(receipt).map_err(|_| SupplyAuthorityError::ReceiptInvalid)?;
        ...
        .await
        .map_err(|_| SupplyAuthorityError::DependencyUnavailable)?;
        let event_digest = sha256(format!("{}:{}", previous.as_deref().unwrap_or("GENESIS"), payload_digest).as_bytes());
        sqlx::query(
        ...
        .await
        .map_err(|_| SupplyAuthorityError::StateConflict)?.rows_affected();
        if affected!=1{return Err(SupplyAuthorityError::OutcomeUnknown);}
        tx.commit().await.map_err(|_| SupplyAuthorityError::OutcomeUnknown)?;
    ...
    pub async fn recover_expired(&self, tenant: Uuid, limit: i64) -> Result<u64, SupplyAuthorityError> {
    ...
    async fn pending_evidence(&self,tenant:Uuid,limit:i64)->Result<Vec<PendingEvidence>,SupplyAuthorityError>{
        if !(1..=100).contains(&limit){return Err(SupplyAuthorityError::RequestInvalid);}
        let mut tx=self.begin_tenant(tenant).await?;
        ...
        tx.commit().await.map_err(|_|SupplyAuthorityError::DependencyUnavailable)?;
        Ok(rows.into_iter().map(|row|PendingEvidence{outbox_id:row.get("outbox_id"),idempotency_key:row.get("idempotency_key"),payload:row.get("payload"),payload_digest:row.get("payload_digest")}).collect())
    }

    async fn mark_evidence_delivered(&self,tenant:Uuid,outbox_id:Uuid,payload_digest:&str,delivery:&AuthorityEvidenceDelivery)->Result<(),SupplyAuthorityError>{
        if !digest(payload_digest)||!digest(&delivery.evidence_digest)||!reference(&delivery.evidence_ref,2048){return Err(SupplyAuthorityError::ReceiptInvalid);}
        let mut tx=self.begin_tenant(tenant).await?;
...
impl SupplyChainAuthority {
    pub fn new(
        ...
        Ok(Self { store, runtime, receipt_keyring, instance_id, lease_seconds })
    }
    ...
    pub async fn execute(
        ...
        if let Some(result) = self.store.prepare_and_claim(
            &request,
            &request_digest,
            &action_hash,
            self.instance_id,
        ...
        let receipt = match self.runtime.execute(&request, &request_digest, &action_hash).await {
            Ok(receipt) => receipt,
            Err(error) => {
                let state = if request.command.operation.requires_external_effect() { "UNKNOWN" } else { "FAILED" };
                let stable = if state == "UNKNOWN" { "SUPPLY_CHAIN_EXTERNAL_OUTCOME_UNKNOWN" } else { error.code() };
                self.store.finish_uncertain(&request, state, stable).await?;
                return Err(if state == "UNKNOWN" { SupplyAuthorityError::OutcomeUnknown } else { error });
            }
        };
        if let Err(error) = self.receipt_keyring.verify(&receipt, &request, &request_digest, Utc::now()) {
            self.store.finish_uncertain(&request, "UNKNOWN", "SUPPLY_CHAIN_RECEIPT_INVALID").await?;
            return Err(error);
        }
        let result=match self.store.commit_success(&request, &request_digest, &receipt).await {
            Ok(result)=>result,
            Err(_)=>{
                ...
                let _=self.store.finish_uncertain(&request,"UNKNOWN","SUPPLY_CHAIN_COMMIT_OUTCOME_UNKNOWN").await;
                return Err(SupplyAuthorityError::OutcomeUnknown);
            }
        };
    ...
    pub async fn authoritative_releases(
        ...
        self.store.authoritative_releases(tenant, limit, cursor).await
    }

    pub async fn flush_evidence(&self,tenant:Uuid,limit:i64)->Result<u64,SupplyAuthorityError>{
        let pending=self.store.pending_evidence(tenant,limit).await?;let mut delivered=0u64;
        for event in pending {
            let receipt=self.runtime.deliver_evidence(tenant,&event.idempotency_key,&event.payload,&event.payload_digest).await?;
            self.store.mark_evidence_delivered(tenant,event.outbox_id,&event.payload_digest,&receipt).await?;
...
fn require_operation(request: &SupplyExecutionRequest, expected: SupplyOperation) -> Result<(), SupplyAuthorityError> {
    if request.command.operation != expected { return Err(SupplyAuthorityError::RequestInvalid); }
    Ok(())
}

fn validate_supply_payload(operation:SupplyOperation,value:&Value,requested_at:DateTime<Utc>)->Result<String,SupplyAuthorityError>{
    let body=value.as_object().ok_or(SupplyAuthorityError::RequestInvalid)?;
    match operation{
        SupplyOperation::Publish=>{
            exact_keys(body,&["manifest","artifact_id","artifact_digest","immutable_reference","sbom_format","sbom_digest","component_count","provenance_digest","source_repository","source_commit","builder_identity","build_definition_digest","license_report_digest","vulnerability_report_digest","maximum_vulnerability","dependency_lock"],&[])?;
            let manifest:DomainPackManifest=serde_json::from_value(body.get("manifest").cloned().ok_or(SupplyAuthorityError::RequestInvalid)?).map_err(|_|SupplyAuthorityError::RequestInvalid)?;
            PackSdk::validate(&manifest).map_err(map_pack_error)?;let artifact_digest=string_field(body,"artifact_digest",64)?;
            let immutable=string_field(body,"immutable_reference",2048)?;let maximum=string_field(body,"maximum_vulnerability",16)?;
            if artifact_digest!=manifest.digest||!digest(&artifact_digest)||uuid_field(body,"artifact_id")?.is_nil()
                ||!immutable.contains(&format!("sha256:{artifact_digest}"))||immutable.to_ascii_lowercase().contains("latest")
                ||!matches!(string_field(body,"sbom_format",32)?.as_str(),"SPDX_JSON"|"CYCLONEDX_JSON")
                ...
                ||!identifier(&string_field(body,"builder_identity",1024)?,1024){return Err(SupplyAuthorityError::RequestInvalid);}
            for key in ["sbom_digest","provenance_digest","build_definition_digest","license_report_digest","vulnerability_report_digest"]{
                if !digest(&string_field(body,key,64)?){return Err(SupplyAuthorityError::RequestInvalid);}
            }
            validate_dependency_lock(body.get("dependency_lock").ok_or(SupplyAuthorityError::RequestInvalid)?)?;
            Ok(format!("pack-release:{}@{}",manifest.pack_id,manifest.version))
        }
        SupplyOperation::Validate=>{
            exact_keys(body,&["run_id","pack_id","version","sandbox_profile_digest","schema_report_digest","dependency_report_digest","vulnerability_report_digest","license_report_digest","behavior_report_digest","threat_report_digest","network_violation_count","conclusion","runner_identity"],&[])?;
            uuid_field(body,"run_id")?;let pack_id=string_field(body,"pack_id",256)?;let version=string_field(body,"version",128)?;
            for key in ["sandbox_profile_digest","schema_report_digest","dependency_report_digest","vulnerability_report_digest","license_report_digest","behavior_report_digest","threat_report_digest"]{
                if !digest(&string_field(body,key,64)?){return Err(SupplyAuthorityError::RequestInvalid);}
            }
            if !valid_semver_string(&version)||!matches!(string_field(body,"conclusion",16)?.as_str(),"PASS"|"FAIL"|"INCONCLUSIVE")
                ||!identifier(&string_field(body,"runner_identity",512)?,512){return Err(SupplyAuthorityError::RequestInvalid);}
            i32_field(body,"network_violation_count",0,1_000_000)?;Ok(format!("pack-release:{pack_id}@{version}"))
        }
        SupplyOperation::Approve=>{
            exact_keys(body,&["approval_id","pack_id","version","manifest_digest","permission_diff","permission_expansion_reviewed","environment","approver_role","principal_assertion_digest","evidence_ref","evidence_digest","expires_at"],&["previous_manifest_digest"])?;
            uuid_field(body,"approval_id")?;let pack_id=string_field(body,"pack_id",256)?;let version=string_field(body,"version",128)?;
            serde_json::from_value::<PermissionDiff>(body.get("permission_diff").cloned().ok_or(SupplyAuthorityError::RequestInvalid)?).map_err(|_|SupplyAuthorityError::RequestInvalid)?;
            let expires=datetime_field(body,"expires_at")?;
            if !valid_semver_string(&version)||!digest(&string_field(body,"manifest_digest",64)?)
                ||optional_string_field(body,"previous_manifest_digest",64)?.is_some_and(|value|!digest(&value))
                ||body.get("permission_expansion_reviewed").and_then(Value::as_bool).is_none()
                ||string_field(body,"environment",64)?!="production"||!identifier(&string_field(body,"approver_role",256)?,256)
                ||!digest(&string_field(body,"principal_assertion_digest",64)?)||!reference(&string_field(body,"evidence_ref",1024)?,1024)
                ||!digest(&string_field(body,"evidence_digest",64)?)||expires<=requested_at||expires>requested_at+Duration::days(366){return Err(SupplyAuthorityError::RequestInvalid);}
            Ok(format!("pack-release:{pack_id}@{version}"))
        }
        SupplyOperation::Activate=>{
            exact_keys(body,&["pack_id","version","environment","manifest_digest","approval_id"],&[])?;
            let pack_id=string_field(body,"pack_id",256)?;let version=string_field(body,"version",128)?;let environment=string_field(body,"environment",64)?;
            if !valid_semver_string(&version)||environment!="production"||!digest(&string_field(body,"manifest_digest",64)?)||uuid_field(body,"approval_id")?.is_nil(){return Err(SupplyAuthorityError::RequestInvalid);}
            Ok(format!("pack-installation:{environment}:{pack_id}"))
        }
        SupplyOperation::Rollback=>{
            exact_keys(body,&["pack_id","environment"],&[])?;let pack_id=string_field(body,"pack_id",256)?;let environment=string_field(body,"environment",64)?;
            if environment!="production"{return Err(SupplyAuthorityError::RequestInvalid);}Ok(format!("pack-installation:{environment}:{pack_id}"))
        }
        SupplyOperation::Revoke|SupplyOperation::Quarantine=>{
            exact_keys(body,&["revocation_id","scope","subject_id","reason_code","running_task_disposition","impact_digest"],&["subject_digest","publisher_id","key_id","pack_id","version"])?;
            uuid_field(body,"revocation_id")?;let scope=string_field(body,"scope",24)?;let subject=string_field(body,"subject_id",640)?;
            if !matches!(string_field(body,"running_task_disposition",20)?.as_str(),"PAUSE"|"KILL"|"ALLOW_TO_FINISH")
                ||!identifier(&string_field(body,"reason_code",128)?,128)||!digest(&string_field(body,"impact_digest",64)?)
                ||optional_string_field(body,"subject_digest",64)?.is_some_and(|value|!digest(&value)){return Err(SupplyAuthorityError::RequestInvalid);}
            match scope.as_str(){
                "PUBLISHER"=>{let publisher=string_field(body,"publisher_id",256)?;if subject!=publisher||body.contains_key("key_id")||body.contains_key("pack_id")||body.contains_key("version"){return Err(SupplyAuthorityError::RequestInvalid);}}
                "KEY"=>{let publisher=string_field(body,"publisher_id",256)?;let key=string_field(body,"key_id",256)?;if subject!=format!("{publisher}#{key}")||body.contains_key("pack_id")||body.contains_key("version"){return Err(SupplyAuthorityError::RequestInvalid);}}
                "ARTIFACT"=>{if !Uuid::parse_str(&subject).is_ok_and(|value|value.to_string()==subject)||body.contains_key("publisher_id")||body.contains_key("key_id")||body.contains_key("pack_id")||body.contains_key("version"){return Err(SupplyAuthorityError::RequestInvalid);}}
                "PACK_RELEASE"=>{let pack=string_field(body,"pack_id",256)?;let version=string_field(body,"version",128)?;if subject!=format!("{pack}@{version}")||!valid_semver_string(&version)||body.contains_key("publisher_id")||body.contains_key("key_id"){return Err(SupplyAuthorityError::RequestInvalid);}}
                _=>return Err(SupplyAuthorityError::RequestInvalid),
            }
            Ok(format!("supply-revocation:{scope}:{subject}"))
        }
        SupplyOperation::Recover=>{
            exact_keys(body,&["unknown_command_id"],&[])?;let command=uuid_field(body,"unknown_command_id")?;Ok(format!("supply-command:{command}"))
        }
    }
}
...
impl PackRegistry for SupplyChainAuthority {
    async fn publish(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Publish)?; self.execute(request).await
    }
    async fn approve(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Approve)?; self.execute(request).await
    }
    async fn activate(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Activate)?; self.execute(request).await
    }
    async fn revoke(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Revoke)?; self.execute(request).await
    }
}
...
impl PackInstaller for SupplyChainAuthority {
    ...
    async fn rollback(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Rollback)?; self.execute(request).await
    }
}
...
impl RevocationService for SupplyChainAuthority {
    async fn quarantine(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Quarantine)?; self.execute(request).await
    }
    async fn revoke_release(&self, request: SupplyExecutionRequest) -> Result<SupplyMutationResult, SupplyAuthorityError> {
        require_operation(&request,SupplyOperation::Revoke)?; self.execute(request).await
    }
    ...
    match request.command.operation {
        SupplyOperation::Publish => {
            ...
            let manifest: DomainPackManifest = serde_json::from_value(
                body.get("manifest").cloned().ok_or(SupplyAuthorityError::RequestInvalid)?,
            ).map_err(|_| SupplyAuthorityError::RequestInvalid)?;
            let artifact_id = uuid_field(body, "artifact_id")?;
            ...
            let manifest_value = serde_json::to_value(&manifest).map_err(|_| SupplyAuthorityError::RequestInvalid)?;
            sqlx::query(
            ...
            let dependency_lock = body.get("dependency_lock").cloned().ok_or(SupplyAuthorityError::RequestInvalid)?;
            ...
            .bind(tenant).bind(&manifest.pack_id).bind(&manifest.version).bind(artifact_id)
            .bind(&manifest_value).bind(&manifest.digest).bind(permission_digest)
            .bind(dependency_lock).bind(dependency_digest)
            .bind(i64::try_from(request.binding.resource_version).map_err(|_| SupplyAuthorityError::RequestInvalid)?)
            .execute(&mut **tx).await.map_err(|_| SupplyAuthorityError::StateConflict)?;
        }
        SupplyOperation::Validate => {
            exact_keys(body,&["run_id","pack_id","version","sandbox_profile_digest","schema_report_digest","dependency_report_digest","vulnerability_report_digest","license_report_digest","behavior_report_digest","threat_report_digest","network_violation_count","conclusion","runner_identity"],&[])?;
            if receipt.sandbox_receipt_digest.is_none()||receipt.vulnerability_receipt_digest.is_none()
                ||receipt.license_receipt_digest.is_none() {
                return Err(SupplyAuthorityError::ReceiptInvalid);
            }
            let conclusion = string_field(body,"conclusion",16)?;
            if !matches!(conclusion.as_str(),"PASS"|"FAIL"|"INCONCLUSIVE") { return Err(SupplyAuthorityError::RequestInvalid); }
            sqlx::query(
            ...
            .bind(tenant).bind(uuid_field(body,"run_id")?).bind(string_field(body,"pack_id",256)?)
            .bind(string_field(body,"version",128)?).bind(string_field(body,"sandbox_profile_digest",64)?)
            .bind(string_field(body,"schema_report_digest",64)?).bind(string_field(body,"dependency_report_digest",64)?)
            .bind(string_field(body,"vulnerability_report_digest",64)?).bind(string_field(body,"license_report_digest",64)?)
            .bind(string_field(body,"behavior_report_digest",64)?).bind(string_field(body,"threat_report_digest",64)?)
            .bind(i32_field(body,"network_violation_count",0,1_000_000)?).bind(&conclusion)
            .bind(string_field(body,"runner_identity",512)?).bind(request.command.requested_at)
            .execute(&mut **tx).await.map_err(|_| SupplyAuthorityError::StateConflict)?;
            if conclusion == "PASS" {
        ...
        SupplyOperation::Approve => {
            exact_keys(body,&["approval_id","pack_id","version","manifest_digest","permission_diff","permission_expansion_reviewed","environment","approver_role","principal_assertion_digest","evidence_ref","evidence_digest","expires_at"],&["previous_manifest_digest"])?;
            let permission_diff: PermissionDiff = serde_json::from_value(body.get("permission_diff").cloned().ok_or(SupplyAuthorityError::RequestInvalid)?)
                .map_err(|_| SupplyAuthorityError::RequestInvalid)?;
            let pack_id=string_field(body,"pack_id",256)?; let version=string_field(body,"version",128)?;
            ...
            let current_manifest:DomainPackManifest=serde_json::from_value(current_manifest_value).map_err(|_|SupplyAuthorityError::StateConflict)?;
            ...
            let prior_permissions=if let Some(row)=previous{
                ...
                if declared_previous.as_deref()!=Some(prior_digest.as_str()){return Err(SupplyAuthorityError::ApprovalDenied);}
                serde_json::from_value::<DomainPackManifest>(row.get("manifest")).map_err(|_|SupplyAuthorityError::StateConflict)?.permissions
            }else{
                if declared_previous.is_some(){return Err(SupplyAuthorityError::ApprovalDenied);}
                crate::PackPermissionDeclaration::default()
            };
            let computed_diff=PermissionDiff::compute(&prior_permissions,&current_manifest.permissions);
            ...
            if !passed||permission_diff!=computed_diff
                ||(computed_diff.expands_privilege()&&body.get("permission_expansion_reviewed")!=Some(&Value::Bool(true))) {
                return Err(SupplyAuthorityError::ApprovalDenied);
            }
            let diff_value=serde_json::to_value(&permission_diff).map_err(|_| SupplyAuthorityError::RequestInvalid)?;
        ...
        SupplyOperation::Activate => {
            exact_keys(body,&["pack_id","version","environment","manifest_digest","approval_id"],&[])?;
            if receipt.installation_receipt_digest.is_none() { return Err(SupplyAuthorityError::ReceiptInvalid); }
            let pack_id=string_field(body,"pack_id",256)?; let version=string_field(body,"version",128)?;
            let environment=string_field(body,"environment",64)?; let manifest_digest=string_field(body,"manifest_digest",64)?;
            let approval_id=uuid_field(body,"approval_id")?;
            let approval: bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM public.supply_chain_pack_approvals WHERE tenant_id=$1 AND approval_id=$2 AND pack_id=$3 AND version=$4 AND manifest_digest=$5 AND environment=$6 AND decision='APPROVED' AND expires_at>now())")
                ...
                .fetch_one(&mut **tx).await.map_err(|_| SupplyAuthorityError::DependencyUnavailable)?;
            if !approval { return Err(SupplyAuthorityError::ApprovalDenied); }
            let installation_affected=sqlx::query("INSERT INTO public.supply_chain_installations
                ...
                .bind(i64::try_from(request.command.expected_resource_version).map_err(|_|SupplyAuthorityError::RequestInvalid)?)
                .execute(&mut **tx).await.map_err(|_| SupplyAuthorityError::StateConflict)?.rows_affected();
            if installation_affected!=1{return Err(SupplyAuthorityError::StateConflict);}
        ...
        SupplyOperation::Rollback => {
            ...
            let installation=sqlx::query("SELECT version,manifest_digest,previous_version,previous_manifest_digest
                ...
                .bind(i64::try_from(request.command.expected_resource_version).map_err(|_|SupplyAuthorityError::RequestInvalid)?)
                .fetch_optional(&mut **tx).await.map_err(|_|SupplyAuthorityError::DependencyUnavailable)?.ok_or(SupplyAuthorityError::StateConflict)?;
            let current_version:String=installation.get("version");let current_manifest:String=installation.get("manifest_digest");
            let target_version:String=installation.get::<Option<String>,_>("previous_version").ok_or(SupplyAuthorityError::RecoveryDenied)?;
            let target_manifest:String=installation.get::<Option<String>,_>("previous_manifest_digest").ok_or(SupplyAuthorityError::RecoveryDenied)?;
            if target_version==current_version&&target_manifest==current_manifest{return Err(SupplyAuthorityError::RecoveryDenied);}
            ...
            if target.get::<String,_>("lifecycle_state")!="ACTIVE"{
                ...
                if release_affected!=1{return Err(SupplyAuthorityError::StateConflict);}
            }
        ...
        SupplyOperation::Revoke | SupplyOperation::Quarantine => {
            exact_keys(body,&["revocation_id","scope","subject_id","reason_code","running_task_disposition","impact_digest"],&["subject_digest","publisher_id","key_id","pack_id","version"])?;
            if receipt.revocation_receipt_digest.is_none() { return Err(SupplyAuthorityError::ReceiptInvalid); }
            let scope=string_field(body,"scope",24)?; let subject_id=string_field(body,"subject_id",1024)?;
            let disposition=string_field(body,"running_task_disposition",20)?;
            if !matches!(scope.as_str(),"PUBLISHER"|"KEY"|"ARTIFACT"|"PACK_RELEASE")
                || !matches!(disposition.as_str(),"PAUSE"|"KILL"|"ALLOW_TO_FINISH") { return Err(SupplyAuthorityError::RequestInvalid); }
            let artifact_state=if request.command.operation==SupplyOperation::Revoke{"REVOKED"}else{"QUARANTINED"};
            let release_state=artifact_state;
            let installation_state=if request.command.operation==SupplyOperation::Revoke{"REVOKED"}else{"PAUSED"};
            match scope.as_str(){
                "PUBLISHER"=>{
                    let publisher_id=string_field(body,"publisher_id",256)?;
                    if subject_id!=publisher_id{return Err(SupplyAuthorityError::RequestInvalid);}
                    ...
                    if !exists{return Err(SupplyAuthorityError::PublisherDenied);}
                    propagate_publisher_safety(tx,tenant,&publisher_id,None,artifact_state,release_state,installation_state).await?;
                }
                "KEY"=>{
                    let publisher_id=string_field(body,"publisher_id",256)?;let key_id=string_field(body,"key_id",256)?;
                    if subject_id!=format!("{publisher_id}#{key_id}"){return Err(SupplyAuthorityError::RequestInvalid);}
                    ...
                    if !exists{return Err(SupplyAuthorityError::PublisherDenied);}
                    propagate_publisher_safety(tx,tenant,&publisher_id,Some(&key_id),artifact_state,release_state,installation_state).await?;
                }
                "ARTIFACT"=>{
                    let artifact_id=Uuid::parse_str(&subject_id).ok().filter(|value|value.to_string()==subject_id).ok_or(SupplyAuthorityError::RequestInvalid)?;
                    ...
                    if !exists{return Err(SupplyAuthorityError::StateConflict);}
                    propagate_artifact_safety(tx,tenant,artifact_id,artifact_state,release_state,installation_state).await?;
                }
                "PACK_RELEASE"=>{
                    let pack_id=string_field(body,"pack_id",256)?;let version=string_field(body,"version",128)?;
                    if subject_id!=format!("{pack_id}@{version}"){return Err(SupplyAuthorityError::RequestInvalid);}
                    let exists:bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM public.supply_chain_pack_releases WHERE tenant_id=$1 AND pack_id=$2 AND version=$3)")
                        .bind(tenant).bind(&pack_id).bind(&version).fetch_one(&mut **tx).await.map_err(|_|SupplyAuthorityError::DependencyUnavailable)?;
                    if !exists{return Err(SupplyAuthorityError::StateConflict);}
                    propagate_release_safety(tx,tenant,&pack_id,&version,release_state,installation_state).await?;
                }
                _=>return Err(SupplyAuthorityError::RequestInvalid),
            }
        ...
        SupplyOperation::Recover => {
            ...
            if receipt.reconciliation_receipt_digest.is_none() { return Err(SupplyAuthorityError::ReceiptInvalid); }
...
async fn propagate_publisher_safety(
    tx:&mut Transaction<'_,Postgres>,tenant:Uuid,publisher_id:&str,key_id:Option<&str>,
    artifact_state:&str,release_state:&str,installation_state:&str,
)->Result<(),SupplyAuthorityError>{
    sqlx::query("UPDATE public.supply_chain_artifact_revisions SET status=$4
        ...
        .bind(tenant).bind(publisher_id).bind(key_id).bind(artifact_state).execute(&mut **tx).await.map_err(|_|SupplyAuthorityError::StateConflict)?;
...
async fn propagate_artifact_safety(
    tx:&mut Transaction<'_,Postgres>,tenant:Uuid,artifact_id:Uuid,
    artifact_state:&str,release_state:&str,installation_state:&str,
)->Result<(),SupplyAuthorityError>{
...
async fn propagate_release_safety(
    tx:&mut Transaction<'_,Postgres>,tenant:Uuid,pack_id:&str,version:&str,
    release_state:&str,installation_state:&str,
)->Result<(),SupplyAuthorityError>{
...
fn result_from_row(row: &sqlx::postgres::PgRow) -> Result<SupplyMutationResult, SupplyAuthorityError> {
    let receipt = row.try_get::<Value, _>("effect_receipt").ok()
        .map(serde_json::from_value).transpose().map_err(|_| SupplyAuthorityError::StateConflict)?;
    Ok(SupplyMutationResult {
        schema_version: SUPPLY_RESULT_SCHEMA.into(),
        command_id: row.get("command_id"),
        state: row.get("state"),
        resource_key: row.get("resource_key"),
        resource_version: u64::try_from(row.get::<i64, _>("resource_version")).map_err(|_| SupplyAuthorityError::StateConflict)?,
...
fn authoritative_release_from_row(row:sqlx::postgres::PgRow)->Result<AuthoritativeRelease,SupplyAuthorityError>{
    let receipt_refs:Vec<SupplyReceiptReference>=serde_json::from_value(row.get::<Value,_>("receipt_refs"))
        .map_err(|_|SupplyAuthorityError::StateConflict)?;
    Ok(AuthoritativeRelease{
        pack_id:row.get("pack_id"),version:row.get("version"),lifecycle_state:row.get("lifecycle_state"),
        resource_version:u64::try_from(row.get::<i64,_>("resource_version")).map_err(|_|SupplyAuthorityError::StateConflict)?,
        manifest_digest:row.get("manifest_digest"),permission_digest:row.get("permission_digest"),
        dependency_lock_digest:row.get("dependency_lock_digest"),artifact_digest:row.get("artifact_digest"),
        immutable_reference:row.get("immutable_reference"),sbom_digest:row.get("sbom_digest"),
        provenance_digest:row.get("provenance_digest"),signature_digest:row.get("signature_digest"),
        license_report_digest:row.get("license_report_digest"),vulnerability_report_digest:row.get("vulnerability_report_digest"),
        maximum_vulnerability:row.get("maximum_vulnerability"),artifact_status:row.get("artifact_status"),receipt_refs,
    })
}

fn decode_release_cursor(value:&str)->Result<(String,String),SupplyAuthorityError>{
    if value.is_empty()||value.len()>2048{return Err(SupplyAuthorityError::RequestInvalid);}
    let raw=URL_SAFE_NO_PAD.decode(value).map_err(|_|SupplyAuthorityError::RequestInvalid)?;
    let fields:Vec<String>=serde_json::from_slice(&raw).map_err(|_|SupplyAuthorityError::RequestInvalid)?;
    if fields.len()!=2||!identifier(&fields[0],256)||!identifier(&fields[1],128){return Err(SupplyAuthorityError::RequestInvalid);}
    Ok((fields[0].clone(),fields[1].clone()))
}

fn encode_release_cursor(pack_id:&str,version:&str)->Result<String,SupplyAuthorityError>{
    serde_jcs::to_vec(&[pack_id,version]).map(|raw|URL_SAFE_NO_PAD.encode(raw)).map_err(|_|SupplyAuthorityError::RequestInvalid)
}

fn required_object_field(value: &Value, key: &str) -> Result<Value, SupplyAuthorityError> {
    value.as_object().and_then(|map| map.get(key)).cloned().ok_or(SupplyAuthorityError::RequestInvalid)
}

fn exact_keys(map:&Map<String,Value>,required:&[&str],optional:&[&str])->Result<(),SupplyAuthorityError>{
    if required.iter().any(|key|!map.contains_key(*key))
        ||map.keys().any(|key|!required.contains(&key.as_str())&&!optional.contains(&key.as_str())){
        return Err(SupplyAuthorityError::RequestInvalid);
    }
    Ok(())
}
...
fn validate_dependency_lock(value:&Value)->Result<(),SupplyAuthorityError>{
    let entries=value.as_array().filter(|entries|!entries.is_empty()&&entries.len()<=1024)
        .ok_or(SupplyAuthorityError::RequestInvalid)?;
    let mut names=BTreeSet::new();
    for entry in entries {
        let object=entry.as_object().ok_or(SupplyAuthorityError::RequestInvalid)?;
        if object.len()!=4 {return Err(SupplyAuthorityError::RequestInvalid);}
        ...
        if !identifier(&name,256)||!identifier(&version,128)||!digest(&digest_value)
            ||reference_value.to_ascii_lowercase().contains("latest")||!reference_value.contains(&format!("sha256:{digest_value}"))
            ||!names.insert(name){return Err(SupplyAuthorityError::RequestInvalid);}
    }
    Ok(())
}

fn string_field(map: &Map<String, Value>, key: &str, maximum: usize) -> Result<String, SupplyAuthorityError> {
    map.get(key).and_then(Value::as_str).filter(|value| !value.is_empty() && value.len()<=maximum && !value.chars().any(char::is_control))
        .map(str::to_string).ok_or(SupplyAuthorityError::RequestInvalid)
}

fn optional_string_field(map: &Map<String, Value>, key: &str, maximum: usize) -> Result<Option<String>, SupplyAuthorityError> {
    match map.get(key) {
...
fn uuid_field(map: &Map<String, Value>, key: &str) -> Result<Uuid, SupplyAuthorityError> {
    string_field(map,key,36).and_then(|value| Uuid::parse_str(&value).ok().filter(|parsed|parsed.to_string()==value).ok_or(SupplyAuthorityError::RequestInvalid))
}
...
fn datetime_field(map: &Map<String, Value>, key: &str) -> Result<DateTime<Utc>, SupplyAuthorityError> {
    string_field(map,key,64).and_then(|value| DateTime::parse_from_rfc3339(&value).map(|time| time.with_timezone(&Utc)).map_err(|_| SupplyAuthorityError::RequestInvalid))
}
...
fn idempotency_key(value: &str) -> bool { (16..=256).contains(&value.len()) && value.bytes().all(|b| b.is_ascii_alphanumeric()||matches!(b,b'.'|b'_'|b':'|b'/'|b'-')) }
fn valid_semver_string(value:&str)->bool{let parts=value.split('.').collect::<Vec<_>>();parts.len()==3&&parts.iter().all(|part|!part.is_empty()&&part.bytes().all(|byte|byte.is_ascii_digit()))}
...
fn json_limits(value: &Value, depth: usize) -> Result<usize, SupplyAuthorityError> {
    if depth>32 { return Err(SupplyAuthorityError::RequestInvalid); }
    match value {
        Value::Null|Value::Bool(_)|Value::Number(_)=>Ok(1),
        Value::String(value) if value.len()<=65_536 && !value.chars().any(char::is_control)=>Ok(value.len()),
        Value::String(_)=>Err(SupplyAuthorityError::RequestInvalid),
        Value::Array(values) if values.len()<=1024=>values.iter().try_fold(0usize,|total,value| json_limits(value,depth+1).and_then(|size| total.checked_add(size).ok_or(SupplyAuthorityError::RequestInvalid))),
        Value::Object(values) if values.len()<=256=>values.iter().try_fold(0usize,|total,(key,value)| {
            if key.is_empty()||key.len()>256||key.chars().any(char::is_control) { return Err(SupplyAuthorityError::RequestInvalid); }
            json_limits(value,depth+1).and_then(|size| total.checked_add(key.len()+size).ok_or(SupplyAuthorityError::RequestInvalid))
        }),
        _=>Err(SupplyAuthorityError::RequestInvalid),
    }.and_then(|size| if size<=1_048_576 {Ok(size)} else {Err(SupplyAuthorityError::RequestInvalid)})
}
...
pub enum SupplyAuthorityError {
    #[error("SUPPLY_CHAIN_REQUEST_INVALID")] RequestInvalid,
    #[error("SUPPLY_CHAIN_BINDING_INVALID")] BindingInvalid,
    #[error("SUPPLY_CHAIN_PRINCIPAL_DENIED")] PrincipalDenied,
    #[error("SUPPLY_CHAIN_PUBLISHER_DENIED")] PublisherDenied,
    #[error("SUPPLY_CHAIN_SIGNATURE_INVALID")] SignatureInvalid,
    #[error("SUPPLY_CHAIN_PACK_INVALID")] PackInvalid,
    #[error("SUPPLY_CHAIN_RECEIPT_INVALID")] ReceiptInvalid,
    #[error("SUPPLY_CHAIN_APPROVAL_DENIED")] ApprovalDenied,
    #[error("SUPPLY_CHAIN_RECOVERY_DENIED")] RecoveryDenied,
    #[error("SUPPLY_CHAIN_IDEMPOTENCY_CONFLICT")] IdempotencyConflict,
    #[error("SUPPLY_CHAIN_STATE_CONFLICT")] StateConflict,
    #[error("SUPPLY_CHAIN_DEPENDENCY_UNAVAILABLE")] DependencyUnavailable,
    #[error("SUPPLY_CHAIN_OUTCOME_UNKNOWN")] OutcomeUnknown,
    #[error("SUPPLY_CHAIN_CONFIGURATION_INVALID")] ConfigurationInvalid,
}

impl SupplyAuthorityError {
    pub fn code(&self) -> &'static str {
        match self {
            Self::RequestInvalid=>"SUPPLY_CHAIN_REQUEST_INVALID", Self::BindingInvalid=>"SUPPLY_CHAIN_BINDING_INVALID",
            Self::PrincipalDenied=>"SUPPLY_CHAIN_PRINCIPAL_DENIED", Self::PublisherDenied=>"SUPPLY_CHAIN_PUBLISHER_DENIED",
            Self::SignatureInvalid=>"SUPPLY_CHAIN_SIGNATURE_INVALID", Self::PackInvalid=>"SUPPLY_CHAIN_PACK_INVALID",
            Self::ReceiptInvalid=>"SUPPLY_CHAIN_RECEIPT_INVALID", Self::ApprovalDenied=>"SUPPLY_CHAIN_APPROVAL_DENIED",
            Self::RecoveryDenied=>"SUPPLY_CHAIN_RECOVERY_DENIED", Self::IdempotencyConflict=>"SUPPLY_CHAIN_IDEMPOTENCY_CONFLICT",
            Self::StateConflict=>"SUPPLY_CHAIN_STATE_CONFLICT", Self::DependencyUnavailable=>"SUPPLY_CHAIN_DEPENDENCY_UNAVAILABLE",
            Self::OutcomeUnknown=>"SUPPLY_CHAIN_OUTCOME_UNKNOWN", Self::ConfigurationInvalid=>"SUPPLY_CHAIN_CONFIGURATION_INVALID",
        }
    }
}
