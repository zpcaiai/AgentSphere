use agent_trust_pack_supply_chain::production::{
    PostgresSupplyChainStore, SupplyChainAuthority, SupplyReceiptKeyring,
};
use agent_trust_pack_supply_chain::server::{
    EvidenceEventKeyring, HttpSupplyChainRuntimePort, SupplyDependency, SupplyServerConfig,
    SupplyTokenAuthorizer, router, serve,
};
...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut database_password =
        read_secret_file("AGENT_TRUST_SUPPLY_DATABASE_PASSWORD_FILE", 16, 8192)?;
    ...
    let evidence_keyring = EvidenceEventKeyring::from_json(&std::fs::read(
        required_private_path("AGENT_TRUST_SUPPLY_EVIDENCE_KEYRING_FILE")?,
    )?)?;
    let evidence_client_identity =
        required_identifier("AGENT_TRUST_SUPPLY_EVIDENCE_CLIENT_IDENTITY")?;
    let runtime = Arc::new(HttpSupplyChainRuntimePort::new(
        outbound,
        coordinator,
        dependencies,
        evidence_keyring,
    ...
    let keyring = SupplyReceiptKeyring::from_json(
        &std::fs::read(required_private_path(
            "AGENT_TRUST_SUPPLY_RECEIPT_KEYRING_FILE",
        )?)?,
        chrono::Utc::now(),
    )?;
    let authority = SupplyChainAuthority::new(
        PostgresSupplyChainStore::new(pool),
        runtime,
        keyring,
    ...
    let tokens = Arc::new(SupplyTokenAuthorizer::from_file(
        &required_private_path("AGENT_TRUST_SUPPLY_TOKEN_BINDINGS_FILE")?,
        &identities,
    )?);
    ...
    serve(
        SupplyServerConfig {
            data_address: SocketAddr::new(
                env::var("AGENT_TRUST_SUPPLY_LISTEN_ADDRESS")?.parse::<IpAddr>()?,
                data_port,
            ),
            management_address: SocketAddr::new(
                env::var("AGENT_TRUST_SUPPLY_MANAGEMENT_LISTEN_ADDRESS")?.parse::<IpAddr>()?,
                management_port,
            ),
...
async fn verify_database_role(
    ...
    {
        return Err("SUPPLY_CHAIN_DATABASE_ROLE_UNSAFE".into());
    }
    Ok(())
}
...
fn database_options(
    ...
    for (key, value) in parsed.query_pairs() {
        ...
        {
            return Err("SUPPLY_CHAIN_DATABASE_URL_INVALID".into());
        }
    }
